...
save(census_total_population, 
     file = "census_total_population.rdata")
...
rm(full_data_for_exclusions)
gc()

pollutants_and_monitors <- 
  counting_data_all[,c("parameter_code",
                       "monitor_id")] %>% 
  unique()
...
monitors_per_pollutant_ordered <- 
  monitors_per_pollutant[
    match(
      components_for_paper, 
      monitors_per_pollutant$parameter_code
      ),
    ]
...
tracts_per_pollutant <- 
  unique(tracts_per_pollutant[,c(1,3)])
...
tracts_per_pollutant_ordered <- 
  tracts_per_pollutant[
    match(
      components_for_paper, 
      tracts_per_pollutant$parameter_code
      ),
    ]
...
# figure out percent above mdl for each species
air_all_obs_mdl_num <- 
  counting_data_all %>%   # this takes observations as the default
  group_by(parameter_code) %>% 
  dplyr::select(parameter_code,
                arithmetic_mean,
                federal_mdl) %>% 
...
nondetects_all_exclusions <- 
  air_all_obs_mdl_num %>% 
  filter(`arithmetic_mean < federal_mdl` == TRUE) %>% 
  arrange(match(parameter_code, 
                components_for_paper))

write.csv(nondetects_all_exclusions, 
          file = 
            "results/nondetects_all_exclusions.csv")
write.csv(air_all_obs_mdl_num, 
          file = 
            "results/air_all_obs_mdl_num.csv")
```
...
```{r counting 3 year data}


# #### three-year 180-day data
# load(file =
#        "air_data_3_year_180_obs_mdl.rdata")
# load(file =
#        "fullcensus.rdata")
# 
# census_total_population <-
...
# 
# # add total population column to main dataset
# three_year_data_for_exclusions <-
#   air_data_3_year_180_obs_mdl %>%
#   left_join(census_total_population,
#             by = c("tract_id" = "tract_ID"),
#             copy = TRUE)
...
#   filter(total_population_DP0010001 > 100)
# 
# rm(three_year_data_for_exclusions)
# gc()
# 
# components_for_paper_plus_ecs <-
#   c("88101",
#     "88104",
#     "88103",
#     "88109",
...
#            components_for_paper_plus_ecs)
# 
# save(counting_data_three,
#      file =
#        "counting_data_three.rdata")

load(file = 
       "counting_data_three.rdata")

pollutants_and_monitors_three <- 
  counting_data_three[,c("parameter_code",
                        "monitor_id")] %>% 
  unique()

# count monitors per pollutant
monitors_per_pollutant_three <- 
  pollutants_and_monitors_three %>% # make the counts
  group_by(parameter_code) %>%
  mutate(count = n())

# total monitors
total_monitors_three <- 
  counting_data_three %>% 
  filter(parameter_code %in% 
           components_for_paper_plus_ecs) %>% 
...
# reduce to parameters and monitors
monitors_per_pollutant_three <- 
  unique(monitors_per_pollutant_three[,c(1,3)])

# make ordered list of monitors per pollutant
monitors_per_pollutant_ordered_three <- 
  monitors_per_pollutant_three[
    match(
      components_for_paper_plus_ecs, 
      monitors_per_pollutant_three$parameter_code
      ),
    ]

write.csv(monitors_per_pollutant_ordered_three,
          file = "results/monitors_per_pollutant_ordered_three.csv")

# count tracts per pollutant in raw data without exclusions
pollutants_and_tracts_three <- 
  counting_data_three[,c("parameter_code",
                                "tract_id")] %>% 
  unique()

tracts_per_pollutant_three <- 
  pollutants_and_tracts_three %>%
  group_by(parameter_code) %>%
  mutate(count = n())

tracts_per_pollutant_three <- 
  unique(tracts_per_pollutant_three[,c(1,3)])

# make ordered list of tracts per pollutant
tracts_per_pollutant_ordered_three <- 
  tracts_per_pollutant_three[
    match(
      components_for_paper_plus_ecs, 
      tracts_per_pollutant_three$parameter_code
      ),
    ]

write.csv(tracts_per_pollutant_ordered_three,
          file = "results/tracts_per_pollutant_ordered_three.csv")

# save files to disk
save(monitors_per_pollutant_ordered_three,
     file = "monitors_per_pollutant_ordered_three.rdata")
save(tracts_per_pollutant_ordered_three,
     file = "tracts_per_pollutant_ordered_three.rdata")
...
# figure out percent above mdl for each species
air_all_obs_mdl_num_three <- 
  counting_data_three %>%   # this takes observations as the default
  group_by(parameter_code) %>% 
  dplyr::select(parameter_code,
                arithmetic_mean,
                federal_mdl) %>% 
...
pm25_non_nas_below_mdl_three <- 
  air_all_obs_mdl_num_three %>% 
  filter(parameter_code == "88101",
           `arithmetic_mean < federal_mdl` == TRUE) %>% 
  pull(n)

pm25_non_nas_above_mdl_three <- 
  air_all_obs_mdl_num_three %>% 
  filter(parameter_code == "88101",
           `arithmetic_mean < federal_mdl` == FALSE) %>% 
  pull(n)
...
air_all_obs_mdl_num_three <- 
  ...
  mutate(n = 
           replace(n, 
                   parameter_code == "88101" & 
                     `arithmetic_mean < federal_mdl` == TRUE, 
                   pm25_non_nas_below_mdl_three + pm25_nas_below_mdl_three)) %>% 
   mutate(n = 
           replace(n, 
                   parameter_code == "88101" & 
                     `arithmetic_mean < federal_mdl` == FALSE, 
                   pm25_non_nas_above_mdl_three + pm25_nas_above_mdl_three)) %>% 
  mutate(n = as.numeric(n)) %>% 
  mutate(percent_of_total = n / observations)

nondetects_all_exclusions_three <- 
  air_all_obs_mdl_num_three %>% 
  filter(`arithmetic_mean < federal_mdl` == TRUE) %>% 
  arrange(match(parameter_code, 
                components_for_paper_plus_ecs))

write.csv(nondetects_all_exclusions_three, 
          file = 
            "results/nondetects_all_exclusions_three.csv")
write.csv(air_all_obs_mdl_num_three, 
          file = 
            "results/air_all_obs_mdl_num_three.csv")

```

```{r make chart for paper}
# make chart for paper
non_detecties <- 
  read_csv("results/nondetects_all_exclusions_three.csv")
...
non_detecties <-  
  ...
  dplyr::select(-X1) %>% 
  add_row( 
    parameter_code = 99321, 
    `arithmetic_mean < federal_mdl` = TRUE, 
    n = n_for_ec, 
    observations = obs_for_ec, 
    percent_of_total = percent_nondetect_ec)

parameter_codes_for_paper <- 
  c("88101",
    "88104",
    "88103",
    "88109",
...
non_detects_for_graph <- 
  non_detecties %>% 
  filter(parameter_code %in% 
           parameter_codes_for_paper) %>% 
  arrange(match(parameter_code, 
                parameter_codes_for_paper)) %>% 
  mutate(title = names_for_paper) %>% 
  dplyr::select(title, parameter_code, percent_of_total)

# make nondetect graph
nondetect_plot <- 
  ggplot(data = non_detects_for_graph, 
       aes(y = percent_of_total*100, 
           x = names_for_paper)) +
  geom_bar(stat = "identity") + 
  ylab("% below MDL") + 
  xlab("component") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.25)) +
  theme(text=element_text(family="Bahnschrift", face="bold", size=12)) + 
  scale_x_discrete(
    limits = names_for_paper,
    labels = 
      c(
expression(paste(PM[2.5])),
"Al",
"As",
"Br",
"Cd",
...
ggsave("plots/nondetect_plot.png", 
       plot = nondetect_plot, 
       height = 10, 
       width = 12, 
       units = "cm")
...
load(file = "counting_data_three.rdata")

monitor_location_data <- 
  counting_data_three %>% 
  dplyr::select(tract_id,
                monitor_id, 
                latitude, 
...
save(monitor_location_data, 
     file = "monitor_location_data.rdata")
load(file = 
       "monitor_location_data.rdata")
...
monitor_location_data_ordered <- 
  full_join(
    datum_order, 
    monitor_location_data, 
    by = "datum")

table(monitor_location_data_ordered$datum)

monitor_location_data_marked <-
  monitor_location_data_ordered %>% 
   distinct(monitor_id,
            parameter_code, 
            .keep_all = TRUE)

save(monitor_location_data_marked, 
     file = "monitor_location_data_marked.rdata")
...
table(monitor_location_data_marked$datum)

monitor_location_data_counts <- 
  monitor_location_data_marked %>% 
...
save(sparse_monitors,
     file = "sparse_monitors.rdata")

table(sparse_monitors$monitor_type)

sparse_monitor_nad83 <- 
  sparse_monitors %>% 
  filter(datum == "NAD83")
save(sparse_monitor_nad83,
     file = "sparse_monitors_nad83.rdata")
  
sparse_monitor_wgs84 <- 
  sparse_monitors %>% 
  filter(datum == "WGS84")
save(sparse_monitor_wgs84,
     file = "sparse_monitors_wgs84.rdata")

sparse_monitor_nad27 <- 
  sparse_monitors %>% 
  filter(datum == "NAD27")
save(sparse_monitor_nad27,
     file = "sparse_monitors_nad27.rdata")

sparse_monitor_who_knows <- 
  sparse_monitors %>% 
  filter(datum == "UNKNOWN")
save(sparse_monitor_who_knows,
     file = "sparse_monitors_who_knows.rdata")

# table(
#   sparse_monitors$datum
# )
# 
# table(
#   sparse_monitors$monitor_type
# )
...
```{r actual mapping monitor locations}

# monitor data

sparse_monitor_nad83_sf <- 
  st_as_sf(
    sparse_monitor_nad83, 
    coords = 
      c("longitude",
...
sparse_monitor_wgs84_to_nad_83_sf <- 
  st_transform(
    sparse_monitor_wgs84_sf, 
    crs = 4269)

sparse_monitor_nad27_to_nad_83_sf <- 
  st_transform(
    sparse_monitor_nad27_sf, 
    crs = 4269)

sparse_monitor_who_knows_to_nad_83_sf <- 
  st_transform(
    sparse_monitor_who_knows_sf, 
    crs = 4269)
...
save(total_pop_sf, 
     file = "total_pop_sf.rdata")

tract_sf <- 
  total_pop_sf

tract_ids_for_checking <- 
  sparse_monitors %>% 
  dplyr::select(tract_id) %>% 
  unique() %>% 
  pull()
...
monitors_sf <- 
  monitors_sf %>% 
  mutate(GEOID = str_pad(tract_id, 11, pad = "0"))

tract_ids_for_checking_padded <- 
  str_pad(tract_ids_for_checking, 
          11, 
          pad = "0")
...
tmap_save(monitor_map, 
          file = "plots/monitor_map.pdf", 
          width = 1000, 
          height = 1500, 
          dpi = 600)

st_write(tract_true_sf, 
         "tract_true_sf.shp")

st_write(monitors_sf, 
         "monitors_sf.shp")

simple_tract_sf <- 
  st_simplify(tract_sf)

ggplot_thing <- 
  ggplot(simple_tract_sf) + 
  geom_sf(aes(fill = 'in_analysis'))
...
```{r count data detritus, eval=FALSE, include=FALSE}
...
```{r mdl analysis, eval=FALSE, include=FALSE}
...
monitorcountonlyurbanover1003_org <- 
  monitorcountonlyurbanover1003 
...
finaldatanodrops$percent_white <- 
  finaldatanodrops$DP0110011 / finaldatanodrops$DP0100001
...
# % Asian
finaldatanodrops$percent_Asian <- finaldatanodrops$DP0110014/finaldatanodrops$DP0100001

# % AIAN/NHOPI
finaldatanodrops$percent_aian_nhopi <- 
  (finaldatanodrops$DP0110013 + 
  finaldatanodrops$DP0110015)/finaldatanodrops$DP0100001
...
```{r make summaries for plots}

# fix component names
load(file = "finaldatanodrops.rdata")

final_data_no_drops_test <- 
  finaldatanodrops
...
final_data_no_drops_test <- 
  final_data_no_drops_test %>% 
  dplyr::rename_at(vars(starts_with('8')), 
                   funs(paste0('c', .))) %>% 
  dplyr::rename_at(vars(starts_with('9')), 
                   funs(paste0('c', .)))
                   
summarizer <- 
  function(component_number) 
  {output <- 
    ddply(final_data_no_drops_test, 
          c("overall_quantile"), 
          summarise,
          N = sum(!is.na(eval(parse(text = component_number)))),
          mean = mean(eval(parse(text = component_number)), na.rm=TRUE),
          sd   = sd(eval(parse(text = component_number)), na.rm=TRUE),
          se   = sd / sqrt(N),
          ciM  = qt(.95/2 + .5, N-1),
          ci   = se*ciM)
  }

final_components_for_paper <- 
  c("88101",
    "88104",
    "88103",
    "88109",
...
final_components_for_paper_with_cs <- 
  paste0("c", 
         final_components_for_paper)

summaries <- 
  map(final_components_for_paper_with_cs, 
      summarizer)
...
pretty_columninator <- 
  function(tibble_name) {
    tibble_name %>% 
      mutate(mean_pretty = formatC(mean, 
                                 format = "fg", 
                                 digits = 4), 
             low_ci_pretty = formatC(mean - ci,
                                     format = "fg",
                                     digits = 4),
             high_ci_pretty = formatC(mean + ci,
                                      format = "fg",
                                      digits = 4),
             n = N,
             magic_column = paste0(mean_pretty,
                                   " (",
                                   low_ci_pretty,
                                   "–",
                                   high_ci_pretty,
...
rows_for_table <- map(summaries,
                      pretty_columninator)

names(rows_for_table) <- 
  component_names

rowmaker <- 
  function(component_name, dataframe) {
      dataframe %>% 
      dplyr::select(magic_column) %>% 
      rename(component_name = magic_column) %>% 
      mutate(quantile = c("1st","2nd","3rd","4th","5th")) %>% 
      melt(id = "quantile") %>% 
      dcast(component_name ~ quantile)
  }

list_of_paper_rows <- 
  map2(component_names, rows_for_table, rowmaker)

export_to_table <- 
  bind_rows(list_of_paper_rows)

write_csv(export_to_table, 
          path = "export_to_table.csv")
...
# pm
pm_plot <- 
  ggplot(summaries$PM2.5, 
         aes(x=factor(overall_quantile), 
             y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
scale_x_discrete(breaks = 1:5, labels=c("1st","2nd","3rd","4th","5th")) +
  xlab("spatial isolation quintile") + 
  ylab(expression(paste("mean concentration (",mu,"g/m"^"3",")"))) +
theme_bw() +
...
ggsave("plots/pm_plot.png",plot=pm_plot,width=20,height=20,units="cm")
...
# al
al_plot<-ggplot(summaries$aluminum, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
ggtitle("Aluminum") +
ggtitle(expression(paste("Aluminum (",italic("n")," = 276)"))) +
geom_text(aes(label=format(mean,digits=2)),hjust=-.1,vjust=0,show.legend=FALSE,family="Bahnschrift",size = 4.5) +
scale_x_discrete(breaks = 1:5, labels=c("1st","2nd","3rd","4th","5th")) +
theme_bw() +
...
ggsave("plots/al_plot.png",plot=al_plot,width=20,height=20,units="cm")


# as
as_plot <- 
  ggplot(summaries$arsenic, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/as_plot.png",plot=as_plot,width=20,height=20,units="cm")


# bromine
br_plot<-ggplot(summaries$bromine, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/br_plot.png",plot=br_plot,width=20,height=20,units="cm")


# cadmium
cd_plot<-ggplot(summaries$cadmium, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
ggtitle(expression(paste("Cadmium (",italic("n")," = 206)"))) +
geom_text(aes(label=format(mean,digits=2)),hjust=-.1,vjust=0,show.legend=FALSE,family="Bahnschrift",size = 4.5) +
scale_x_discrete(breaks = 1:5, labels=c("1st","2nd","3rd","4th","5th")) +
theme_bw() +
theme(text=element_text(family="Bahnschrift", size = 18)) +
...
ggsave("plots/cd_plot.png",plot=cd_plot,width=20,height=20,units="cm")


# calcium
ca_plot<-ggplot(summaries$calcium, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/ca_plot.png",plot=ca_plot,width=20,height=20,units="cm")


# chlorine
cl_plot<-ggplot(summaries$chlorine, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/cl_plot.png",plot=cl_plot,width=20,height=20,units="cm")


# copper
cu_plot<-ggplot(summaries$copper, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/cu_plot.png",plot=cu_plot,width=20,height=20,units="cm")


# iron
fe_plot<-ggplot(summaries$iron, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/fe_plot.png",plot=fe_plot,width=20,height=20,units="cm")


# lead
pb_plot<-ggplot(summaries$lead, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/pb_plot.png",plot=pb_plot,width=20,height=20,units="cm")


# mercury
hg_plot<-ggplot(summaries$mercury, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
ggtitle(expression(paste("Mercury (",italic("n")," = 162)"))) +
geom_text(aes(label=format(mean,digits = 2)),hjust=-.1,vjust=1,show.legend=FALSE,family="Bahnschrift",size = 4.5) +
scale_x_discrete(breaks = 1:5, labels=c("1st","2nd","3rd","4th","5th")) +
theme_bw() +
theme(text=element_text(family="Bahnschrift", size = 18)) +
theme(plot.title = element_text(size = 18,hjust = 0.5)) +
...
ggsave("plots/hg_plot.png",plot=hg_plot,width=20,height=20,units="cm")


# nickel
ni_plot<-ggplot(summaries$nickel, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/ni_plot.png",plot=ni_plot,width=20,height=20,units="cm")


# silicon
si_plot<-ggplot(summaries$silicon, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/si_plot.png",plot=si_plot,width=20,height=20,units="cm")


# sodium
na_plot<-ggplot(summaries$sodium, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/na_plot.png",plot=na_plot,width=20,height=20,units="cm")


# titanium
ti_plot<-ggplot(summaries$titanium, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/ti_plot.png",plot=ti_plot,width=20,height=20,units="cm")


# vanadium plot
v_plot<-ggplot(summaries$vanadium, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/v_plot.png",plot=v_plot,width=20,height=20,units="cm")


# zinc plot
zn_plot<-ggplot(summaries$zinc, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/zn_plot.png",plot=zn_plot,width=20,height=20,units="cm")


# ammonium
am_plot<-ggplot(summaries$ammonium_ion, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
ggtitle("Aluminum") +
...
ggsave("plots/am_plot.png",plot=am_plot,width=20,height=20,units="cm")


# sodium ion plot
nai_plot<-ggplot(summaries$sodium_ion, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/nai_plot.png",plot=nai_plot,width=20,height=20,units="cm")


# nitrate
no_plot<-ggplot(summaries$nitrate_ion, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/no_plot.png",plot=no_plot,width=20,height=20,units="cm")


# sulfate
so_plot<-ggplot(summaries$sulfate_ion, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
ggtitle(expression(paste("Sulfate Ion (",italic("n")," = 277)"))) +
geom_text(aes(label=format(mean,digits = 3)),hjust=-.1,vjust=0,show.legend=FALSE,family="Bahnschrift",size = 4.5) +
scale_x_discrete(breaks = 1:5, labels=c("1st","2nd","3rd","4th","5th")) +
theme_bw() +
theme(text=element_text(family="Bahnschrift", size = 18)) +
theme(plot.title = element_text(size = 18,hjust = 0.5)) +
...
ggsave("plots/so_plot.png",plot=so_plot,width=20,height=20,units="cm")


# elemental carbon
ec_plot<-ggplot(summaries$elemental_carbon, aes(x=factor(overall_quantile), y=mean,color=overall_quantile)) +
scale_color_gradientn(colors = rainbow(4)) +
geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, size=.7) +
geom_point() +
...
ggsave("plots/ec_plot.png",
       plot=ec_plot,width=20,height=20,units="cm")



# plot all the plots together
all_plots <- 
  grid.arrange(rbind(ggplotGrob(al_plot), 
                     ggplotGrob(ca_plot),
                     ggplotGrob(pb_plot),
                     ggplotGrob(ti_plot),
                     ggplotGrob(nai_plot),
                     size="last"),
               rbind(ggplotGrob(as_plot),
                     ggplotGrob(cl_plot), 
                     ggplotGrob(hg_plot), 
                     ggplotGrob(v_plot),
                     ggplotGrob(no_plot),
                     size="last"),
               rbind(ggplotGrob(br_plot), 
                     ggplotGrob(cu_plot), 
                     ggplotGrob(ni_plot), 
                     ggplotGrob(zn_plot),
                     ggplotGrob(so_plot), 
                     size = "last"),
               rbind(ggplotGrob(cd_plot), 
                     ggplotGrob(fe_plot), 
                     ggplotGrob(si_plot), 
                     ggplotGrob(am_plot),
                     ggplotGrob(ec_plot), 
...
pm25_88101_levene <- 
  leveneTest(finaldatanodrops$`88101` ~
               as.factor(finaldatanodrops$overall_quantile))
pm25_88101_levene

write_csv(pm25_88101_levene, 
          path = "results/anova/pm25_88101_levene.csv")
          
# one-way anova
pm25_88101_one <- 
  oneway.test(finaldatanodrops$`88101` ~ 
                finaldatanodrops$overall_quantile)
pm25_88101_one

lapply(pm25_88101_one, 
       write, 
       "results/anova/pm25_88101_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
pm25_88101_no_nas <- 
  finaldatanodrops[c("88101",
                     "overall_quantile")] %>% 
  na.omit()

pm25_88101_games_howell <- 
  games.howell(pm25_88101_no_nas$overall_quantile,
               pm25_88101_no_nas$`88101`)
pm25_88101_games_howell

write_csv(pm25_88101_games_howell, 
          path = "results/anova/pm25_88101_games_howell.csv")
...
# levene's test unequal variances
al_88104_levene <- 
  leveneTest(finaldatanodrops$`88104` ~
               as.factor(finaldatanodrops$overall_quantile))
al_88104_levene

write_csv(al_88104_levene, 
          path = "results/anova/al_88104_levene.csv")
          
# one-way anova
al_88104_one <- 
  oneway.test(finaldatanodrops$`88104` ~ 
                finaldatanodrops$overall_quantile)
al_88104_one

lapply(al_88104_one, 
       write, 
       "results/anova/al_88104_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
al_88104_no_nas <- 
  finaldatanodrops[c("88104",
                     "overall_quantile")] %>% 
  na.omit()

al_88104_games_howell <- 
  games.howell(al_88104_no_nas$overall_quantile,
               al_88104_no_nas$`88104`)
al_88104_games_howell

write_csv(al_88104_games_howell, 
          path = "results/anova/al_88104_games_howell.csv")
...
# levene's test unequal variances
as_88103_levene <- 
  leveneTest(finaldatanodrops$`88103` ~
               as.factor(finaldatanodrops$overall_quantile))
as_88103_levene

write_csv(as_88103_levene, 
          path = "results/anova/as_88103_levene.csv")
          
# one-way anova
as_88103_one <- 
  oneway.test(finaldatanodrops$`88103` ~ 
                finaldatanodrops$overall_quantile)
as_88103_one

lapply(as_88103_one, 
       write, 
       "results/anova/as_88103_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
as_88103_no_nas <- 
  finaldatanodrops[c("88103",
                     "overall_quantile")] %>% 
  na.omit()

as_88103_games_howell <- 
  games.howell(as_88103_no_nas$overall_quantile,
               as_88103_no_nas$`88103`)
as_88103_games_howell

write_csv(as_88103_games_howell, 
          path = "results/anova/as_88103_games_howell.csv")
...
# levene's test unequal variances
br_88109_levene <- 
  leveneTest(finaldatanodrops$`88109` ~
               as.factor(finaldatanodrops$overall_quantile))
br_88109_levene

write_csv(br_88109_levene, 
          path = "results/anova/br_88109_levene.csv")
          
# one-way anova
br_88109_one <- 
  oneway.test(finaldatanodrops$`88109` ~ 
                finaldatanodrops$overall_quantile)
br_88109_one

lapply(br_88109_one, 
       write, 
       "results/anova/br_88109_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
br_88109_no_nas <- 
  finaldatanodrops[c("88109",
                     "overall_quantile")] %>% 
  na.omit()

br_88109_games_howell <- 
  games.howell(br_88109_no_nas$overall_quantile,
               br_88109_no_nas$`88109`)
br_88109_games_howell$`upper limit`

write_csv(br_88109_games_howell, 
          path = "results/anova/br_88109_games_howell.csv")
...
# levene's test unequal variances
cd_88110_levene <- 
  leveneTest(finaldatanodrops$`88110` ~
               as.factor(finaldatanodrops$overall_quantile))
cd_88110_levene

write_csv(cd_88110_levene, 
          path = "results/anova/cd_88110_levene.csv")
          
# one-way anova
cd_88110_anova <- 
  aov(finaldatanodrops$`88110` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(cd_88110_anova)

cd_88110_one <- 
  oneway.test(finaldatanodrops$`88110` ~ 
                finaldatanodrops$overall_quantile)
cd_88110_one

lapply(cd_88110_one, 
       write, 
       "results/anova/cd_88110_one.txt", 
       append=TRUE, 
       ncolumns=1000)
...
# games-howell
cd_88110_no_nas <- 
  finaldatanodrops[c("88110",
                     "overall_quantile")] %>% 
  na.omit()

cd_88110_games_howell <- 
  games.howell(cd_88110_no_nas$overall_quantile,
               cd_88110_no_nas$`88110`)
cd_88110_games_howell

write_csv(cd_88110_games_howell, 
          path = "results/anova/cd_88110_games_howell.csv")
...
# levene's test unequal variances
ca_88111_levene <- 
  leveneTest(finaldatanodrops$`88111` ~
               as.factor(finaldatanodrops$overall_quantile))
ca_88111_levene

write_csv(ca_88111_levene, 
          path = "results/anova/ca_88111_levene.csv")
          
# one-way anova
ca_88111_anova <- 
  aov(finaldatanodrops$`88111` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(ca_88111_anova)

ca_88111_one <- 
  oneway.test(finaldatanodrops$`88111` ~ 
                finaldatanodrops$overall_quantile)
ca_88111_one

lapply(ca_88111_one, 
       write, 
       "results/anova/ca_88111_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
ca_88111_no_nas <- 
  finaldatanodrops[c("88111",
                     "overall_quantile")] %>% 
  na.omit()

ca_88111_games_howell <- 
  games.howell(ca_88111_no_nas$overall_quantile,
               ca_88111_no_nas$`88111`)
ca_88111_games_howell

write_csv(ca_88111_games_howell, 
          path = "results/anova/ca_88111_games_howell.csv")
...
# levene's test unequal variances
cl_88115_levene <- 
  leveneTest(finaldatanodrops$`88115` ~
               as.factor(finaldatanodrops$overall_quantile))
cl_88115_levene

write_csv(cl_88115_levene, 
          path = "results/anova/cl_88115_levene.csv")
          
# one-way anova
cl_88115_anova <- 
  aov(finaldatanodrops$`88115` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(cl_88115_anova)

cl_88115_one <- 
  oneway.test(finaldatanodrops$`88115` ~ 
                finaldatanodrops$overall_quantile)
cl_88115_one

lapply(cl_88115_one, 
       write, 
       "results/anova/cl_88115_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
cl_88115_no_nas <- 
  finaldatanodrops[c("88115",
                     "overall_quantile")] %>% 
  na.omit()

cl_88115_games_howell <- 
  games.howell(cl_88115_no_nas$overall_quantile,
               cl_88115_no_nas$`88115`)
cl_88115_games_howell

write_csv(cl_88115_games_howell, 
          path = "results/anova/cl_88115_games_howell.csv")
...
# levene's test unequal variances
cu_88114_levene <- 
  leveneTest(finaldatanodrops$`88114` ~
               as.factor(finaldatanodrops$overall_quantile))
cu_88114_levene

write_csv(cu_88114_levene, 
          path = "results/anova/cu_88114_levene.csv")
          
# one-way anova
cu_88114_anova <- 
  aov(finaldatanodrops$`88114` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(cu_88114_anova)

cu_88114_one <- 
  oneway.test(finaldatanodrops$`88114` ~ 
                finaldatanodrops$overall_quantile)
cu_88114_one

lapply(cu_88114_one, 
       write, 
       "results/anova/cu_88114_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
cu_88114_no_nas <- 
  finaldatanodrops[c("88114",
                     "overall_quantile")] %>% 
  na.omit()

cu_88114_games_howell <- 
  games.howell(cu_88114_no_nas$overall_quantile,
               cu_88114_no_nas$`88114`)
cu_88114_games_howell

write_csv(cu_88114_games_howell, 
          path = "results/anova/cu_88114_games_howell.csv")
...
# levene's test unequal variances
fe_88126_levene <- 
  leveneTest(finaldatanodrops$`88126` ~
               as.factor(finaldatanodrops$overall_quantile))
fe_88126_levene

write_csv(fe_88126_levene, 
          path = "results/anova/fe_88126_levene.csv")
          
# one-way anova
fe_88126_anova <- 
  aov(finaldatanodrops$`88126` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(fe_88126_anova)

fe_88126_one <- 
  oneway.test(finaldatanodrops$`88126` ~ 
                finaldatanodrops$overall_quantile)
fe_88126_one

lapply(fe_88126_one, 
       write, 
       "results/anova/fe_88126_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
fe_88126_no_nas <- 
  finaldatanodrops[c("88126",
                     "overall_quantile")] %>% 
  na.omit()

fe_88126_games_howell <- 
  games.howell(fe_88126_no_nas$overall_quantile,
               fe_88126_no_nas$`88126`)
fe_88126_games_howell

write_csv(fe_88126_games_howell, 
          path = "results/anova/fe_88126_games_howell.csv")
...
# levene's test unequal variances
pb_88128_levene <- 
  leveneTest(finaldatanodrops$`88128` ~
               as.factor(finaldatanodrops$overall_quantile))
pb_88128_levene

write_csv(pb_88128_levene, 
          path = "results/anova/pb_88128_levene.csv")
          
# one-way anova
pb_88128_anova <- 
  aov(finaldatanodrops$`88128` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(pb_88128_anova)

pb_88128_one <- 
  oneway.test(finaldatanodrops$`88128` ~ 
                finaldatanodrops$overall_quantile)
pb_88128_one

lapply(pb_88128_one, 
       write, 
       "results/anova/pb_88128_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
pb_88128_no_nas <- 
  finaldatanodrops[c("88128",
                     "overall_quantile")] %>% 
  na.omit()

pb_88128_games_howell <- 
  games.howell(pb_88128_no_nas$overall_quantile,
               pb_88128_no_nas$`88128`)
pb_88128_games_howell

write_csv(pb_88128_games_howell, 
          path = "results/anova/pb_88128_games_howell.csv")
...
# levene's test unequal variances
hg_88142_levene <- 
  leveneTest(finaldatanodrops$`88142` ~
               as.factor(finaldatanodrops$overall_quantile))
hg_88142_levene

write_csv(hg_88142_levene, 
          path = "results/anova/hg_88142_levene.csv")
          
# one-way anova
hg_88142_anova <- 
  aov(finaldatanodrops$`88142` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(hg_88142_anova)

hg_88142_one <- 
  oneway.test(finaldatanodrops$`88142` ~ 
                finaldatanodrops$overall_quantile)
hg_88142_one

lapply(hg_88142_one, 
       write, 
       "results/anova/hg_88142_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
hg_88142_no_nas <- 
  finaldatanodrops[c("88142",
                     "overall_quantile")] %>% 
  na.omit()

hg_88142_games_howell <- 
  games.howell(hg_88142_no_nas$overall_quantile,
               hg_88142_no_nas$`88142`)
hg_88142_games_howell

write_csv(hg_88142_games_howell, 
          path = "results/anova/hg_88142_games_howell.csv")
...
# levene's test unequal variances
ni_88136_levene <- 
  leveneTest(finaldatanodrops$`88136` ~
               as.factor(finaldatanodrops$overall_quantile))
ni_88136_levene

write_csv(ni_88136_levene, 
          path = "results/anova/ni_88136_levene.csv")
          
# one-way anova
ni_88136_anova <- 
  aov(finaldatanodrops$`88136` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(ni_88136_anova)

ni_88136_one <- 
  oneway.test(finaldatanodrops$`88136` ~ 
                finaldatanodrops$overall_quantile)
ni_88136_one

lapply(ni_88136_one, 
       write, 
       "results/anova/ni_88136_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
ni_88136_no_nas <- 
  finaldatanodrops[c("88136",
                     "overall_quantile")] %>% 
  na.omit()

ni_88136_games_howell <- 
  games.howell(ni_88136_no_nas$overall_quantile,
               ni_88136_no_nas$`88136`)
ni_88136_games_howell

write_csv(ni_88136_games_howell, 
          path = "results/anova/ni_88136_games_howell.csv")
...
# levene's test unequal variances
si_88165_levene <- 
  leveneTest(finaldatanodrops$`88165` ~
               as.factor(finaldatanodrops$overall_quantile))
si_88165_levene

write_csv(si_88165_levene, 
          path = "results/anova/si_88165_levene.csv")
          
# one-way anova
si_88165_one <- 
  oneway.test(finaldatanodrops$`88165` ~ 
                finaldatanodrops$overall_quantile)
si_88165_one

lapply(si_88165_one, 
       write, 
       "results/anova/si_88165_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
si_88165_no_nas <- 
  finaldatanodrops[c("88165",
                     "overall_quantile")] %>% 
  na.omit()

si_88165_games_howell <- 
  games.howell(si_88165_no_nas$overall_quantile,
               si_88165_no_nas$`88165`)
si_88165_games_howell

write_csv(si_88165_games_howell, 
          path = "results/anova/si_88165_games_howell.csv")
...
# levene's test unequal variances
na_88184_levene <- 
  leveneTest(finaldatanodrops$`88184` ~
               as.factor(finaldatanodrops$overall_quantile))
na_88184_levene

write_csv(na_88184_levene, 
          path = "results/anova/na_88184_levene.csv")
          
# one-way anova
na_88184_anova <- 
  aov(finaldatanodrops$`88184` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(na_88184_anova)

na_88184_one <- 
  oneway.test(finaldatanodrops$`88184` ~ 
                finaldatanodrops$overall_quantile)
na_88184_one

lapply(na_88184_one, 
       write, 
       "results/anova/na_88184_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
na_88184_no_nas <- 
  finaldatanodrops[c("88184",
                     "overall_quantile")] %>% 
  na.omit()

na_88184_games_howell <- 
  games.howell(na_88184_no_nas$overall_quantile,
               na_88184_no_nas$`88184`)
na_88184_games_howell

write_csv(na_88184_games_howell, 
          path = "results/anova/na_88184_games_howell.csv")
...
# levene's test unequal variances
ti_88161_levene <- 
  leveneTest(finaldatanodrops$`88161` ~
               as.factor(finaldatanodrops$overall_quantile))
ti_88161_levene

write_csv(ti_88161_levene, 
          path = "results/anova/ti_88161_levene.csv")
          
# one-way anova
ti_88161_one <- 
  oneway.test(finaldatanodrops$`88161` ~ 
                finaldatanodrops$overall_quantile)
ti_88161_one

lapply(ti_88161_one, 
       write, 
       "results/anova/ti_88161_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
ti_88161_no_nas <- 
  finaldatanodrops[c("88161",
                     "overall_quantile")] %>% 
  na.omit()

ti_88161_games_howell <- 
  games.howell(ti_88161_no_nas$overall_quantile,
               ti_88161_no_nas$`88161`)
ti_88161_games_howell

write_csv(ti_88161_games_howell, 
          path = "results/anova/ti_88161_games_howell.csv")
...
# levene's test unequal variances
v_88164_levene <- 
  leveneTest(finaldatanodrops$`88164` ~
               as.factor(finaldatanodrops$overall_quantile))
v_88164_levene

write_csv(v_88164_levene, 
          path = "results/anova/v_88164_levene.csv")
          
# one-way anova
v_88164_anova <- 
  aov(finaldatanodrops$`88164` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(v_88164_anova)

v_88164_one <- 
  oneway.test(finaldatanodrops$`88164` ~ 
                finaldatanodrops$overall_quantile)
v_88164_one

lapply(v_88164_one, 
       write, 
       "results/anova/v_88164_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
v_88164_no_nas <- 
  finaldatanodrops[c("88164",
                     "overall_quantile")] %>% 
  na.omit()

v_88164_games_howell <- 
  games.howell(v_88164_no_nas$overall_quantile,
               v_88164_no_nas$`88164`)
v_88164_games_howell

write_csv(v_88164_games_howell, 
          path = "results/anova/v_88164_games_howell.csv")
...
# levene's test unequal variances
zn_88167_levene <- 
  leveneTest(finaldatanodrops$`88167` ~
               as.factor(finaldatanodrops$overall_quantile))
zn_88167_levene

write_csv(zn_88167_levene, 
          path = "results/anova/zn_88167_levene.csv")
          
# one-way anova
zn_88167_anova <- 
  aov(finaldatanodrops$`88167` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(zn_88167_anova)

zn_88167_one <- 
  oneway.test(finaldatanodrops$`88167` ~ 
                finaldatanodrops$overall_quantile)
zn_88167_one

lapply(zn_88167_one, 
       write, 
       "results/anova/zn_88167_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
zn_88167_no_nas <- 
  finaldatanodrops[c("88167",
                     "overall_quantile")] %>% 
  na.omit()

zn_88167_games_howell <- 
  games.howell(zn_88167_no_nas$overall_quantile,
               zn_88167_no_nas$`88167`)
zn_88167_games_howell

write_csv(zn_88167_games_howell, 
          path = "results/anova/zn_88167_games_howell.csv")
...
# levene's test unequal variances
am_88301_levene <- 
  leveneTest(finaldatanodrops$`88301` ~
               as.factor(finaldatanodrops$overall_quantile))
am_88301_levene

write_csv(am_88301_levene, 
          path = "results/anova/am_88301_levene.csv")
          
# one-way anova
am_88301_anova <- 
  aov(finaldatanodrops$`88301` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(am_88301_anova)

am_88301_one <- 
  oneway.test(finaldatanodrops$`88301` ~ 
                finaldatanodrops$overall_quantile)
am_88301_one

lapply(am_88301_one, 
       write, 
       "results/anova/am_88301_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
am_88301_no_nas <- 
  finaldatanodrops[c("88301",
                     "overall_quantile")] %>% 
  na.omit()

am_88301_games_howell <- 
  games.howell(am_88301_no_nas$overall_quantile,
               am_88301_no_nas$`88301`)
am_88301_games_howell

write_csv(am_88301_games_howell, 
          path = "results/anova/am_88301_games_howell.csv")
...
# levene's test unequal variances
nai_88302_levene <- 
  leveneTest(finaldatanodrops$`88302` ~
               as.factor(finaldatanodrops$overall_quantile))
nai_88302_levene

write_csv(nai_88302_levene, 
          path = "results/anova/nai_88302_levene.csv")
          
# one-way anova
nai_88302_anova <- 
  aov(finaldatanodrops$`88302` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(nai_88302_anova)

nai_88302_one <- 
  oneway.test(finaldatanodrops$`88302` ~ 
                finaldatanodrops$overall_quantile)
nai_88302_one

lapply(nai_88302_one, 
       write, 
       "results/anova/nai_88302_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
nai_88302_no_nas <- 
  finaldatanodrops[c("88302",
                     "overall_quantile")] %>% 
  na.omit()

nai_88302_games_howell <- 
  games.howell(nai_88302_no_nas$overall_quantile,
               nai_88302_no_nas$`88302`)
nai_88302_games_howell

write_csv(nai_88302_games_howell, 
          path = "results/anova/nai_88302_games_howell.csv")
...
# levene's test unequal variances
no_88306_levene <- 
  leveneTest(finaldatanodrops$`88306` ~
               as.factor(finaldatanodrops$overall_quantile))
no_88306_levene

write_csv(no_88306_levene, 
          path = "results/anova/no_88306_levene.csv")
          
# one-way anova
no_88306_anova <- 
  aov(finaldatanodrops$`88306` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(no_88306_anova)

no_88306_one <- 
  oneway.test(finaldatanodrops$`88306` ~ 
                finaldatanodrops$overall_quantile)
no_88306_one

lapply(no_88306_one, 
       write, 
       "results/anova/no_88306_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
no_88306_no_nas <- 
  finaldatanodrops[c("88306",
                     "overall_quantile")] %>% 
  na.omit()

no_88306_games_howell <- 
  games.howell(no_88306_no_nas$overall_quantile,
               no_88306_no_nas$`88306`)
no_88306_games_howell

write_csv(no_88306_games_howell, 
          path = "results/anova/no_88306_games_howell.csv")
...
# levene's test unequal variances
so_88403_levene <- 
  leveneTest(finaldatanodrops$`88403` ~
               as.factor(finaldatanodrops$overall_quantile))
so_88403_levene

write_csv(so_88403_levene, 
          path = "results/anova/so_88403_levene.csv")
          
# one-way anova
so_88403_one <- 
  oneway.test(finaldatanodrops$`88403` ~ 
                finaldatanodrops$overall_quantile)
so_88403_one

lapply(so_88403_one, 
       write, 
       "results/anova/so_88403_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
so_88403_no_nas <- 
  finaldatanodrops[c("88403",
                     "overall_quantile")] %>% 
  na.omit()

so_88403_games_howell <- 
  games.howell(so_88403_no_nas$overall_quantile,
               so_88403_no_nas$`88403`)
so_88403_games_howell

write_csv(so_88403_games_howell, 
          path = "results/anova/so_88403_games_howell.csv")

```


```{r elemental carbon anova}

# levene's test unequal variances
ec_99321_levene <- 
  leveneTest(finaldatanodrops$`99321` ~
               as.factor(finaldatanodrops$overall_quantile))
ec_99321_levene

write_csv(ec_99321_levene, 
          path = "results/anova/ec_99321_levene.csv")
          
# one-way anova
ec_99321_anova <- 
  aov(finaldatanodrops$`99321` ~ 
        as.factor(finaldatanodrops$overall_quantile))
summary(ec_99321_anova)

ec_99321_one <- 
  oneway.test(finaldatanodrops$`99321` ~ 
                finaldatanodrops$overall_quantile)
ec_99321_one

lapply(ec_99321_one, 
       write, 
       "results/anova/ec_99321_one.txt", 
       append=TRUE, 
       ncolumns=1000)

# games-howell
ec_99321_no_nas <- 
  finaldatanodrops[c("99321",
                     "overall_quantile")] %>% 
  na.omit()

ec_99321_games_howell <- 
  games.howell(ec_99321_no_nas$overall_quantile,
               ec_99321_no_nas$`99321`)
ec_99321_games_howell

write_csv(ec_99321_games_howell, 
          path = "results/anova/ec_99321_games_howell.csv")
...
cor(data_for_correlation, method = "spearman", 
    use = "pairwise.complete.obs")

glm_maker <- 
  function(variable, model_name, digities = 4) {
    model <- 
      glm(
        data = finaldatanodrops,
        variable ~ 
          overall_quantile +
          percent_black + 
          percent_Hispanic + 
...
variable_names_text <- 
  paste0("finaldatanodrops$`",
         final_components_for_paper, 
         "`")

View(variable_names_text)

write_file(variable_names_text, 
          path = "variable_names.txt")

variables_for_glm <- 
  c(finaldatanodrops$`88101`,
  finaldatanodrops$`88104`,
finaldatanodrops$`88103`,
finaldatanodrops$`88109`,
finaldatanodrops$`88110`,
finaldatanodrops$`88111`,
finaldatanodrops$`88115`,
finaldatanodrops$`88114`,
finaldatanodrops$`88126`,
finaldatanodrops$`88128`,
finaldatanodrops$`88142`,
finaldatanodrops$`88136`,
finaldatanodrops$`88165`,
finaldatanodrops$`88184`,
finaldatanodrops$`88161`,
finaldatanodrops$`88164`,
finaldatanodrops$`88167`,
finaldatanodrops$`88301`,
finaldatanodrops$`88302`,
finaldatanodrops$`88306`,
finaldatanodrops$`88403`,
finaldatanodrops$`99321`)

map2()
  
glm_maker(finaldatanodrops$`88101`, 
          "pm_25_glm", 3)
glm_maker(finaldatanodrops$`88104`, 
          "aluminum_glm", 5)
glm_maker(finaldatanodrops$`88103`, 
          "arsenic_glm", 7)
glm_maker(finaldatanodrops$`88109`, 
          "bromine_glm", 6)
glm_maker(finaldatanodrops$`88110`,
          "cadmium_glm", 5)
glm_maker(finaldatanodrops$`88111`, 
          "calcium_glm", 4)
glm_maker(finaldatanodrops$`88115`, 
          "chlorine_glm")
glm_maker(finaldatanodrops$`88114`, 
          "copper_glm", 4)
glm_maker(finaldatanodrops$`88126`, 
          "iron_glm", 4)
glm_maker(finaldatanodrops$`88128`,
          "lead_glm", 4)
glm_maker(finaldatanodrops$`88142`,
          "mercury_glm", 5)
glm_maker(finaldatanodrops$`88136`,
          "nickel_glm", 6)
glm_maker(finaldatanodrops$`88165`,
          "silicon_glm", 4)
glm_maker(finaldatanodrops$`88184`,
          "sodium_glm", 5)
glm_maker(finaldatanodrops$`88161`,
          "titanium_glm", 6)
glm_maker(finaldatanodrops$`88164`,
          "vanadium_glm", 6)
glm_maker(finaldatanodrops$`88167`,
          "zinc_glm", 5)
glm_maker(finaldatanodrops$`88301`,
          "ammonium_ion_glm", 4)
glm_maker(finaldatanodrops$`88302`,
          "sodium_ion_glm", 5)
glm_maker(finaldatanodrops$`88306`,
          "nitrate_ion_glm", 3)
glm_maker(finaldatanodrops$`88403`,
          "sulfate_ion_glm", 3)
glm_maker(finaldatanodrops$`99321`, 
          "elemental_carbon_glm")


# pm2.5
pm_25_glm <- 
  glm(data = finaldatanodrops, 
      `88101` ~ 
        overall_quantile +
        percent_black + 
        percent_Hispanic + 
...
tab_model(br_glm, 
          file = "glm_tables/br_glm.doc", 
          digits = 4)



al_88104_glm <- 
  glm(finaldatanodrops$`88104` ~ 
        finaldatanodrops$overall_quantile + 
        finaldatanodrops$percent_less_than_high_school + 
        finaldatanodrops$percent_poverty + 
        finaldatanodrops$percent_Hispanic + 
        finaldatanodrops$percent_black + 
        finaldatanodrops$percent_white +
        finaldatanodrops$Region)

summary(al_88104_glm)

tab_model(al_88104_glm, 
          file = "glm_tables/al_88104_glm.doc", 
          digits = 3)
...
# simple glm
simpleglmPM25 <- glm(finaldatanodrops$`88101`~finaldatanodrops$overall_quantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region+finaldatanodrops$percent_black)
