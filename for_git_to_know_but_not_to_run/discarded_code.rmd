...
```{r snap to grid minimizer detritus}
...
# for each erroring tract, run it until 

snap_to_gridder <- 
  function(buffer_sf, cell_size) {
    snapped_buffers <- 
      st_snap_to_grid(buffers,
                      size = cell_size) 
    
    
    
    # run the interesections
    safely(buffer_intersector_tract(snapped_buffers))
...
```{r deleted whatnot from comparing errors}
...
success_detector_for_tract_list <- 
  function(intersect_output_tract_list){
  map(.f = success_detector, 
      .x = intersect_output_tract_list)
  }

precision_successes <- 
  map(.x = precise_tracts_intersections_only, 
      .f = success_detector_for_tract_list)
...
```{r make tracts valid and then intersect buffers}
...
load(file = file.path("intermediate_data", "erroring_tracts.rdata"))
...
# save erroring_tracts_valid and remove erroring tracts
save(erroring_tracts_valid, 
     file = file.path("intermediate_data", "erroring_tracts_valid.rdata"))
gc()

mem_used()

# safely intersect all buffer sizes for all erroring tracts
valid_intersected_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = erroring_tracts_valid)
gc()
mem_used()

# remove erroring_tracts_valid
rm(erroring_tracts_valid)
gc()
mem_used()

# investigate errors
intersected_valid_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = valid_intersected_tracts)

save(valid_intersected_tracts, 
     file = file.path("intermediate_data", 
                      "valid_intersected_tracts.rdata"))
rm(valid_intersected_tracts)
gc()
mem_used()
...
save(valid_error_counts, 
     file = file.path("intermediate_data", "valid_error_counts.rdata"))
save(valid_error_indices, 
     file = file.path("intermediate_data", "valid_error_indices.rdata"))
...
```{r buffer tracts by 0 and then intersect buffers}
...
# do the buffering
erroring_tracts_zero_buffered <-
  map(.x = erroring_tracts,
      .f = zero_bufferer)
gc()

# save erroring_tracts_zero_buffered and remove erroring_tracts
save(erroring_tracts_zero_buffered, 
     file = file.path("intermediate_data", 
                      "erroring_tracts_zero_buffered.rdata")) 
rm(erroring_tracts)
gc()

# safely intersect all buffer sizes for all erroring tracts
zero_buffered_intersected_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = erroring_tracts_zero_buffered)
gc()

# remove erroring_tracts_zero_buffered
rm(erroring_tracts_zero_buffered)
gc()

# investigate errors
intersected_zero_buffered_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = zero_buffered_intersected_tracts)

# count errors
zero_buffered_error_counts <-
  map(
    .x = intersected_zero_buffered_errors,
    .f = length
  )

# get buffer indices of tracts still erroring
zero_buffered_error_indices <-
  map(
    .f = names,
    .x = intersected_zero_buffered_errors
  )

save(zero_buffered_intersected_tracts, 
     file = file.path("intermediate_data", 
                      "zero_buffered_intersected_tracts.rdata"))
save(zero_buffered_error_counts, 
     file = file.path("intermediate_data", "zero_buffered_error_counts.rdata"))
save(zero_buffered_error_indices, 
     file = file.path("intermediate_data", "zero_buffered_error_indices.rdata"))
...
# valid indices
indices_use_from_valid <- valid_successes

# zero buffered indices
indices_use_from_zero_buffered <- 
  map2(.x = zero_buffered_successes, 
       .y = valid_successes, 
       .f = setdiff)  # takes elements in the first but not in the second

indices_use_from_valid_or_zero_buffered <- 
  map2(.x = indices_use_from_valid, 
       .y = indices_use_from_zero_buffered, 
       .f = c)  # concatenates the lists




tracts_use_from_valid <- 
  map2(.x = valid_intersected_tracts, 
       .y = indices_use_from_valid, 
       .f = function(x, y) x[y])
  
tracts_use_from_zero_buffered <-
  map2(.x = zero_buffered_intersected_tracts, 
       .y = indices_use_from_zero_buffered, 
       .f = function(x, y) x[y])
