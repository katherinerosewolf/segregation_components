##############
#Code prepared by Katherine Wolf
#2017-08-06
##############
...
setwd("C:/Rfiles/MEScProjectR/WorkingData/")

#### load raw data ####
# census
fullcensus=read.csv("fullcensus.csv", header = TRUE) # load data
fullcensus$GEOID10<-as.character(fullcensus$GEOID10) # make ID into character
...
# SPEC_filenames<-list.files(pattern = "daily_SPEC_.*\\.csv")
# SPEC_file_list<-lapply(SPEC_filenames,function(i){
#   read.csv(file=i, header=TRUE)
# })
# names(SPEC_file_list) <- gsub("\\.csv$", "", SPEC_filenames)
# 
# # import total PM25 88101 files
# PM_25_88101_filenames<-list.files(pattern = "daily_88101_.*\\.csv")
# PM_25_88101_file_list<-lapply(PM_25_88101_filenames,function(i){
#   read.csv(file=i, header=TRUE)
# })
# names(PM_25_88101_file_list) <- gsub("\\.csv$", "", PM_25_88101_filenames)
# 
# # import total PM25 88502 files
# PM_25_88502_filenames<-list.files(pattern = "daily_88502_.*\\.csv")
# PM_25_88502_file_list<-lapply(PM_25_88502_filenames,function(i){
#   read.csv(file=i, header=TRUE)
# })
# names(PM_25_88502_file_list) <- gsub("\\.csv$", "", PM_25_88502_filenames)
# 
# # save original files to disk
# list.save(SPEC_file_list, file = "SPEC_file_list_unedited.rdata")  #save list to disk
# list.save(PM_25_88101_file_list, file = "PM_25_88101_file_list_unedited.rdata")  #save list to disk
# list.save(PM_25_88502_file_list, file = "PM_25_88502_file_list_unedited.rdata")  #save list to disk
# 
# 
# 
# 
# ####START HERE TO AVOID RE-READING FILES
# #SPEC_file_list<-list.load(file = "SPEC_file_list_unedited.rdata")
# #PM_25_88101_file_list<-list.load(file = "PM_25_88101_file_list_unedited.rdata")
# #PM_25_88502_file_list<-list.load(file = "PM_25_88502_file_list_unedited.rdata")
# 
# # fix state codes for speciation data from 2005 to 2007
# # force state code and state name to be character vectors
# SPEC_file_list<-lapply(SPEC_file_list, function(x) {x$State.Code <- as.character(x$State.Code); x})
# PM_25_88101_file_list<-lapply(PM_25_88101_file_list, function(x) {x$State.Code <- as.character(x$State.Code); x})
# PM_25_88502_file_list<-lapply(PM_25_88502_file_list, function(x) {x$State.Code <- as.character(x$State.Code); x})
#  
# SPEC_file_list<-lapply(SPEC_file_list, function(x) {x$State.Name <- as.character(x$State.Name); x})
# PM_25_88101_file_list<-lapply(PM_25_88101_file_list, function(x) {x$State.Name <- as.character(x$State.Name); x})
# PM_25_88502_file_list<-lapply(PM_25_88502_file_list, function(x) {x$State.Name <- as.character(x$State.Name); x})
#  
# 
# 
# # replace CC with 81 in state code for Canada
# # FRM
# PM_25_88101_file_list<-lapply(PM_25_88101_file_list, function(x){
#  x$State.Code<-replace(x$State.Code,x$State.Code=="CC","81");x
#  }
#  )
# # non-FRM
# # replace CC with 81 in state code for Canada
# PM_25_88502_file_list<-lapply(PM_25_88502_file_list, function(x){
#   x$State.Code<-replace(x$State.Code,x$State.Code=="CC","81");x
#   }
#   )
# 
...
# states_and_counties<-read.csv(file="states_and_counties.csv")
# states_and_codes<-unique(states_and_counties[c("State.Name","State.Code")])
# states_and_codes$State.Code<-as.character(states_and_codes$State.Code)
# states_and_codes$State.Name<-as.character(states_and_codes$State.Name)
# 
...
# SPEC_file_list<-lapply(SPEC_file_list, function(x) {x$State.Code <- as.numeric(x$State.Code); x})
# PM_25_88101_file_list<-lapply(PM_25_88101_file_list, function(x) {x$State.Code <- as.numeric(x$State.Code); x})
# PM_25_88502_file_list<-lapply(PM_25_88502_file_list, function(x) {x$State.Code <- as.numeric(x$State.Code); x})
# 
# 
# # create site IDs to match Census files #
# SPEC_file_list<-lapply(SPEC_file_list, function(x) {
#   x$ID<-paste(formatC(x$State.Code, width=2, flag="0"),
#                formatC(x$County.Code, width=3, flag="0"),
#                formatC(x$Site.Num, width=4, flag="0"), sep="-");x})
# 
# PM_25_88101_file_list<-lapply(PM_25_88101_file_list, function(x) {
#   x$ID<-paste(formatC(x$State.Code, width=2, flag="0"),
#               formatC(x$County.Code, width=3, flag="0"),
...
# SPEC_file_list<-lapply(SPEC_file_list, function(x) {x$ID<-as.factor(x$ID); x})
# PM_25_88101_file_list<-lapply(PM_25_88101_file_list, function(x) {x$ID<-as.factor(x$ID); x})
# PM_25_88502_file_list<-lapply(PM_25_88502_file_list, function(x) {x$ID<-as.factor(x$ID); x})
# 
# # make parameter code a factor
# SPEC_file_list<-lapply(SPEC_file_list, function(x) {x$Parameter.Code<-as.factor(x$Parameter.Code); x})
# PM_25_88101_file_list<-lapply(PM_25_88101_file_list, function(x) {x$Parameter.Code<-as.factor(x$Parameter.Code); x})
# PM_25_88502_file_list<-lapply(PM_25_88502_file_list, function(x) {x$Parameter.Code<-as.factor(x$Parameter.Code); x})
# 
# 
# # save all lists to disk
# list.save(SPEC_file_list, file = "SPEC_file_list.rdata")  #save list to disk
# list.save(PM_25_88101_file_list, file = "PM_25_88101_file_list.rdata")  #save list to disk
# list.save(PM_25_88502_file_list, file = "PM_25_88502_file_list.rdata")  #save list to disk
# 
# # pull PM2.5
# PM25FRM<-do.call("rbind", PM_25_88101_file_list)
# PM25nonFRM<-do.call("rbind", PM_25_88502_file_list)
# PM25all<-rbind(PM25FRM,PM25nonFRM)
# 
# # pull species
# speciesoptions<-as.list(levels(SPEC_file_list$daily_SPEC_2005$Parameter.Name))
# 
# # get speciation parameter codes
# species_parameter_codes<-read.csv("speciation_parameter_codes.csv", header = TRUE)
# species_parameter_codes_only<-species_parameter_codes[,c("Parameter","Parameter.Code")]
# species_parameter_codes_only <- unique(species_parameter_codes_only)
# species_parameter_codes_only$Parameter.Code<-as.factor(species_parameter_codes_only$Parameter.Code)
# 
# # pull individual species
# aluminum_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88104)
# ammonium_ion_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88301)
# antimony_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88102)
# arsenic_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88103)
# barium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88107)
# beryllium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88105)
# carbon_black_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88313)
# bromine_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88109)
# cadmium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88110)
# calcium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88111)
# cerium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88117)
# cesium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88118)
# chloride_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88203)
# chlorine_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88115)
# chromium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88112)
# cobalt_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88113)
# copper_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88114)
# carbon_EC_CSN_LC_TOT_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88307)
# carbon_EC_LC_TOR_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88321)
# carbon_EC_LC_TOT_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88381)
# carbon_EC_OPTICAL_TOT_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88316)
# europium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88121)
# gallium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88124)
# gold_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88143)
# hafnium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88127)
# indium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88131)
# iridium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88133)
# iron_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88126)
# lanthanum_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88146)
# lead_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88128)
# magnesium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88140)
# manganese_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88132)
# mercury_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88142)
# molybdenum_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88134)
# nickel_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88136)
# niobium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88147)
# nitrate_non_volatile_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88310)
# carbon_OC_TOR_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88320)
# carbon_OC_TOT_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88382)
# carbon_OCX_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88304)
# PM_25_SPEC_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88101)
# phosphorus_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88152)
# potassium_ion_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88303)
# potassium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88180)
# rubidium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88176)
# samarium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88162)
# scandium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88163)
# selenium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88154)
# silicon_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88165)
# silver_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88166)
# sodium_ion_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88302)
# sodium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88184)
# strontium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88168)
# sulfate_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88403)
# sulfur_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88169)
# tantalum_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88170)
# terbium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88172)
# tin_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88160)
# titanium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88161)
# carbon_TC_LC_TOT_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88312)
# nitrate_total_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88306)
# tungsten_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88186)
# carbon_UV_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88314)
# uranium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88179)
# vanadium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88164)
# nitrate_volatile_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88309)
# yttrium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88183)
# zinc_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88167)
# zirconium_by_year<-lapply(SPEC_file_list, subset, Parameter.Code == 88185)
# 
# # put them all in a list
# species_list_by_year<-mget(ls(pattern = "_by_year"))
# 
# # save above list to disk
# list.save(species_list_by_year, file = "species_list_by_year.rdata")
# 
# # combine each sub-list into one dataframe
# species_list<-lapply(species_list_by_year, function(i){
#   rbindlist(i)
# })
# 
# # put all species in one dataframe for monitor extraction
# species_list_dataframe<-lapply(species_list,function(i){ # make each a dataframe
#   as.data.frame(i)
# })
# massive_species_dataframe<-rbindlist(species_list_dataframe) #combine them!
# View(massive_species_dataframe)
# View(PM25all)
# 
# 
# 
# # save dataframes to disk before edits
# list.save(species_list,file="species_list.rdata")
# save(massive_species_dataframe,file="massive_species_dataframe.rdata")
# save(PM25FRM,file="PM25FRM.rdata")
# save(PM25nonFRM,file="PM25nonFRM.rdata")
# save(PM25all,file="PM25all.rdata")
# 
# rm("SPEC_file_list","species_list_by_year","species_list_dataframe")
# 
# 
...
# 
# # SPEC_file_list<-list.load(file = "SPEC_file_list.rdata")
# # PM_25_88101_file_list<-list.load(file = "PM_25_88101_file_list.rdata")
# # PM_25_88502_file_list<-list.load(file = "PM_25_88502_file_list.rdata")
...
# # data cleaning
# 
# # rechecking for state code errors
# massive_species_dataframe_states<-massive_species_dataframe[,c("State.Code","State.Name")]
# 
# # check state speciation assignments by year
# SPEC_state_counts<-massive_species_dataframe_states %>%
#     group_by(State.Code, State.Name) %>%
#     summarise (n = n())
# 
# SPEC_state_counts<-massive_species_dataframe_states %>%
...
#   summarise (n = n())
# 
# #check state PM assignments
# PM_states<-PM25all[,c("State.Code","State.Name")]
# 
# PM_state_counts<-PM_states %>%
#   group_by(State.Code, State.Name) %>%
#   summarise (n = n())
# 
# 
# 
# # generating unique site ID lists
# 
# # speciation monitors!
# 
# # dataframe of speciation monitors
# speciation_monitor_preliminary_data<-massive_species_dataframe[,c("State.Code","County.Code","Site.Num","Latitude","Longitude","Datum","Local.Site.Name","Address","State.Name","County.Name","City.Name","CBSA.Name","ID")]
# speciation_monitor_preliminary_data_dataframe<-as.data.frame(speciation_monitor_preliminary_data) #make it a df
# speciation_monitors_no_exact_dups<-unique(speciation_monitor_preliminary_data_dataframe) # take only unique monitors
# View(speciation_monitors_no_exact_dups)
# 
# # drop unused factor levels for all columns for speciation monitors
# speciation_monitors_no_exact_dups<-droplevels(speciation_monitors_no_exact_dups)
...
# speciation_only_dups<-speciation_monitors_no_exact_dups[duplicated(speciation_monitors_no_exact_dups$ID)| duplicated(speciation_monitors_no_exact_dups$ID, fromLast=TRUE),]
...
# write.csv(monitors_NAD83, "C:/ArcGIS/MEScProjectGIS/MonitorAssignmentsJanuary2018/monitors_NAD83.csv")
# write.csv(monitors_WGS84, "C:/ArcGIS/MEScProjectGIS/MonitorAssignmentsJanuary2018/monitors_WGS84.csv")
# write.csv(monitors_NAD27, "C:/ArcGIS/MEScProjectGIS/MonitorAssignmentsJanuary2018/monitors_NAD27.csv")
# write.csv(monitors_UNKNO, "C:/ArcGIS/MEScProjectGIS/MonitorAssignmentsJanuary2018/monitors_UNKNO.csv")
# 
# #### MUST STOP HERE AND ASSIGN THE MONTIORS TO TRACTS
# 
# # perhaps put a load step here?
# 
# # read in the table with monitors assigned to census tracts
# monitors_assigned_to_tracts_raw<-read.csv(file="monitors_assigned_to_tracts.txt",header=TRUE)
# View(monitors_assigned_to_tracts_raw)
# 
# #only pull information needed from each line
# monitors_assigned_to_tracts<-monitors_assigned_to_tracts_raw
# 
# #rename GEOID10 to tract_ID and coerce to character
# monitors_assigned_to_tracts$tract_ID<-as.character(monitors_assigned_to_tracts$GEOID10)
# monitors_assigned_to_tracts$GEOID10<-NULL
# 
# #rename ID to monitor ID
# monitors_assigned_to_tracts$monitor_ID<-as.character(monitors_assigned_to_tracts$ID)
# monitors_assigned_to_tracts$ID<-NULL
# 
# # count monitors
# spec_monitor_count<-table(monitors_assigned_to_tracts$speciation)
# View(spec_monitor_count)
# PM_monitor_count<-table(monitors_assigned_to_tracts$PM25FRM,monitors_assigned_to_tracts$PM25nonFRM)
# View(PM_monitor_count)
# 
# # view list of monitors occupying the same census tract
# # find duplicated rows for speciation monitors
# multiple_monitors_in_single_tracts<-monitors_assigned_to_tracts[duplicated(monitors_assigned_to_tracts$GEOID10)| duplicated(monitors_assigned_to_tracts$GEOID10, fromLast=TRUE),]
# View(multiple_monitors_in_single_tracts)
# 
# # just pull monitor IDs and census tracts
# monitors_and_tracts<-monitors_assigned_to_tracts[,c("tract_ID","monitor_ID")]
# View(monitors_and_tracts)
# 
# # pull monitor IDs, census tracts, and monitor types
# monitors_tracts_types<-monitors_assigned_to_tracts[,c("tract_ID","monitor_ID","PM25FRM","PM25nonFRM","speciation")]
# View(monitors_tracts_types)
# 
# # make dataframe with all air data in one place
# complete_air_data<-rbind(massive_species_dataframe,PM25all)
# View(complete_air_data)
# 
# # clarify monitor ID
# complete_air_data$monitor_ID<-as.character(complete_air_data$ID)
# complete_air_data$ID<-NULL
# 
# # drop extra factor levels
# droplevels(complete_air_data)
# 
# # save complete air data to disk
# save(complete_air_data,file="complete_air_data.rdata")
# 
# ##### works to here
# 
# # join tracts to full data
# complete_air_and_tracts<-merge(complete_air_data,monitors_tracts_types,by="monitor_ID",all=TRUE)
# View(complete_air_and_tracts)
# 
# # save to disk
# save(complete_air_and_tracts,file="complete_air_and_tracts.rdata")
# 
# # make it a little smaller
# compact_air_and_tracts<-subset(complete_air_and_tracts,select = -c(AQI,CBSA.Name))
# object.size(compact_air_and_tracts)
# 
# # save to disk
# save(compact_air_and_tracts,file="compact_air_and_tracts.rdata")
# 
# # remove others to save working memory
# rm(complete_air_data)
# 
# # ##### skip if doing a full run, or start here if doing partial run?
# #
# # load(file="complete_air_and_tracts.rdata")
# # load(file="compact_air_and_tracts.rdata")
# # ##### end skipping
# #
# # make parameter code a character for subsetting
# compact_air_and_tracts$Parameter.Code<-as.character(compact_air_and_tracts$Parameter.Code)
# 
...
#   "88502")
# 
# elements_and_codes<-data.frame(component,parameter_code)
# elements_and_codes
# write.csv(elements_and_codes,file="elements_and_codes.csv")
# elements_and_codes<-read.csv(file="elements_and_codes.csv")
# View(elements_and_codes)
# 
# # break out individual parameter codes into lists
# elements_and_codes$parameter_code<-as.character(elements_and_codes$parameter_code)
# compact_air_and_tracts$Parameter.Code<-as.character(compact_air_and_tracts$Parameter.Code)
# components<-lapply(elements_and_codes$parameter_code,function(i){
#   subset(compact_air_and_tracts,Parameter.Code==i)
#          })
# names(components)<-elements_and_codes$parameter_code
# 
# # save broken-out components
# list.save(components,file="components.rdata")
...
# onlytheonesIwant <- do.call("rbind", components)
# save(onlytheonesIwant,file="onlytheonesIwant.rdata")
...
# aggregate(monitor_ID ~ Parameter.Name, onlytheonesIwant, function(x) length(unique(x)))
# 
# ##### converting data types
# 
# #####
# 
# # convert TOT to TOR for EC by multiplying by 1.3
# 
# datatobecorrected<-onlytheonesIwant
# rm(onlytheonesIwant)
# 
# 
# #####
...
# allairdatacorrectedEC88381<-rbind(datatobecorrected,datatobecorrected[Parameter.Code==88381][,c('Arithmetic.Mean','Parameter.Code') := list(Arithmetic.Mean*1.3,"99381")])
#   
# # change those coded as 88307 to 99307 in output from prior step
# allairdatacorrectedEC88307and88381<-rbind(allairdatacorrectedEC88381,allairdatacorrectedEC88381[Parameter.Code==88307][,c('Arithmetic.Mean','Parameter.Code') := list(Arithmetic.Mean*1.3,"99307")])
# 
# # change those coded as 88316 to 99316 in output from prior step
# allairdatacorrectedEC88316and88307and88381<-rbind(allairdatacorrectedEC88307and88381,allairdatacorrectedEC88307and88381[Parameter.Code==88316][,c('Arithmetic.Mean','Parameter.Code') := list(Arithmetic.Mean*1.3,"99316")])
# 
# # check the above for a sample date
# # View(allairdatacorrectedEC88316and88307and88381[allairdatacorrectedEC88316and88307and88381$Parameter.Code %in% c(88316, 99316),])
# 
# 
# # make one new EC code for all EC types called 99321
# allairdatacorrectedEC<-rbind(allairdatacorrectedEC88316and88307and88381,allairdatacorrectedEC88316and88307and88381[allairdatacorrectedEC88316and88307and88381$Parameter.Code %in% c("88321","99381","99307","99316"),][,c('Arithmetic.Mean','Parameter.Code') := list(Arithmetic.Mean,"99321")])
# 
# # check the above for a sample date
# # View(allairdatacorrectedEC[which(allairdatacorrectedEC$Parameter.Code %in% c("88321","88316","88381","88307","99321") & allairdatacorrectedEC$Date.Local == "2010-01-02")])
# 
# 
# # make a table of counts per parameter code
# table(allairdatacorrectedEC$Parameter.Code)
# 
# # remove interim files to save memory
# rm(allairdatacorrectedEC88316and88307and88381,allairdatacorrectedEC88307and88381,allairdatacorrectedEC88381)
# 
# 
# # make one new PM2.5 code, 99101, for both FRM and non-FRM
# allairdatacorrected<-rbind(allairdatacorrectedEC,allairdatacorrectedEC[allairdatacorrectedEC$Parameter.Code %in% c("88101","88502"),][,c('Arithmetic.Mean','Parameter.Code') := list(Arithmetic.Mean,"99101")])


# # check the above for a sample date
# View(allairdatacorrected[which(allairdatacorrected$Parameter.Code %in% c("88101","88502","99101") & allairdatacorrected$Date.Local == "2010-01-02")])
# 
# # remove extraneous files to save memory
# rm(allairdatacorrectedEC,datatobecorrected)
# 
# # save allairdatacorrected to disk
# save(allairdatacorrected,file="allairdatacorrected.rdata")
# 
# #clear memory
# gc()

##### POLLUTION SECTION ENDS HERE!

#####
# # rurality
# # load allairdatacorrected if not running straight
# load(file="allairdatacorrected.rdata")
# 
# # read in data
# rucacodes<-read.csv(file="ruca2010.csv")
# 
...
# 
# # remove non-states
# allairdataandruca<-allairdataandruca[!(is.na(allairdataandruca$urban))]
# 
# # format dates
# 
# # convert to date if not already
# allairdataandruca$Date.Local <- as.Date(allairdataandruca$Date.Local)
# str(allairdataandruca$Date.Local)
# 
# #get months
# allairdataandruca$month <- format(allairdataandruca$Date.Local,format="%m")
# 
# #get years
# allairdataandruca$year <- format(allairdataandruca$Date.Local,format="%y")
# 
# # remove no-longer-needed files from memory
# rm(allairdatacorrected,whataretheseNAs,onlyruca,rucacodes)
# 
# # save allairdataandruca to disk
# save(allairdataandruca,file="allairdataandruca.rdata")
# gc()
# 
#
# #### tract and monitor counts overall ####
# 
# # load allairdataandruca if not running straight
# load(file="allairdataandruca.rdata")
# 
# # count observations
# countthisdata<-allairdataandruca %>%
#   group_by(Parameter.Code, tract_ID) %>%
#   mutate(count = n())
# 
# countthisdatasimple<-countthisdata[,c("Parameter.Code","tract_ID","count")]
# countthisdatasimple<-unique(countthisdatasimple[,1:3])
# View(countthisdatasimple)
# 
# # count monitors per pollutant in raw data without exclusions
# pollutantsandmonitors<-allairdataandruca[,c("Parameter.Code","monitor_ID")] # select only data points needed
# View(pollutantsandmonitors)
# pollutantsandmonitors<-unique(pollutantsandmonitors)
# View(pollutantsandmonitors)
# 
# monitorsperpollutant<-pollutantsandmonitors %>% # make the counts
#   group_by(Parameter.Code) %>%
#   mutate(count = n())
# monitorsperpollutant<-unique(monitorsperpollutant[,c(1,3)])
# View(monitorsperpollutant)
# 
# # count tracts per pollutant in raw data without exclusions
# pollutantsandtracts<-allairdataandruca[,c("Parameter.Code","tract_ID")]
# pollutantsandtracts<-unique(pollutantsandtracts)
# View(pollutantsandtracts)
# tractsperpollutant<-pollutantsandtracts %>%
#   group_by(Parameter.Code) %>%
#   mutate(count = n())
# View(tractsperpollutant)
# tractsperpollutant<-unique(tractsperpollutant[,c(1,3)])
# View(tractsperpollutant)
# 
# # save files to disk
# save(countthisdata,file="countthisdata.rdata")
# save(countthisdatasimple,file="countthisdatasimple.rdata")
# save(monitorsperpollutant,file="monitorsperpollutant.rdata")
# save(tractsperpollutant,file="tractsperpollutant.rdata")
# 
...
# save(monitorsperpollutant,file="results/monitorsperpollutant.rdata")
# write.csv(monitorsperpollutant,file="results/monitorsperpollutant.csv")
# save(tractsperpollutant,file="results/tractsperpollutant.rdata")
# write.csv(tractsperpollutant,file="results/tractperpollutant.csv")
# 
# # remove unneeded files
# rm(pollutantsandtracts,pollutantsandmonitors,countthisdatasimple,allairdataandruca)
# gc()
...
#
#
# #### CREATE FILES FOR SENSITIVITY ANALYSES ####
# 
...
# atleast300obs<-countthisdata[which(countthisdata$count >=300),]
# 
# # find first and last date for each pollutant for each tract
# atleast300obs %>%
#   group_by(Parameter.Code, tract_ID) %>%
#   mutate(FirstDate = min(Date.Local), LastDate = max(Date.Local)) -> atleast300obsfirstlastdate
# rm(atleast300obs)
# gc()
# 
# # subtract them
# atleast300obsfirstlastdate$date_difference <- atleast300obsfirstlastdate$LastDate - atleast300obsfirstlastdate$FirstDate
# gc()
# 
# # delete rows where there aren't enough days
# atleast5years300obs<-atleast300obsfirstlastdate[which(atleast300obsfirstlastdate$date_difference >= 1825),]
# rm(atleast300obsfirstlastdate)
# gc()
# 
# # save final file to disk and then remove for later loading
# save(atleast5years300obs,file="atleast5years300obs.rdata")
...
# atleast180obs<-countthisdata[which(countthisdata$count >=180),]
# 
# # find first and last date for each pollutant for each tract
# atleast180obs %>%
#   group_by(Parameter.Code, tract_ID) %>%
#   mutate(FirstDate = min(Date.Local), LastDate = max(Date.Local)) -> atleast180obsfirstlastdate
# rm(atleast180obs)
# gc()
# 
# # subtract them
# atleast180obsfirstlastdate$date_difference <- atleast180obsfirstlastdate$LastDate - atleast180obsfirstlastdate$FirstDate
# gc()
# 
# # delete rows where there aren't enough days
# atleast3years180obs<-atleast180obsfirstlastdate[which(atleast180obsfirstlastdate$date_difference >= 1095),]
# rm(atleast180obsfirstlastdate)
# gc()
#
# # save final file to disk and then remove for later loading
# save(atleast3years180obs,file="atleast3years180obs.rdata")
...
censusadjacent=read.csv("censusadjacent.csv", header = TRUE)
censusadjacent<-censusadjacent[c(-1)]
censusadjacent$NODE_COUNT<-as.numeric(censusadjacent$NODE_COUNT)

# import census data (Black, Black/other, total)
segregationvars<-fullcensus[c("GEOID10", "DP0080001", "DP0080004")]
segregationvars$allbutblack<-segregationvars$DP0080001-segregationvars$DP0080004 # create non-black
colnames(segregationvars)[1] <- "selfneighbor"
...
require(data.table)
canncon<-setDT(censusadjacentnonodes)[,.(nbr_GEOID10=paste(nbr_GEOID10, collapse = ",")), by=src_GEOID10]
canncon$src_GEOID10<-as.character(canncon$src_GEOID10)

# add own census tract to tracts
canncon$selfandfriends<-paste(canncon$src_GEOID10, canncon$nbr_GEOID10, sep=",")
str(canncon)

# save file to disk
save(canncon,file="canncon.rdata")
...
censusadjacentyesnodes$src_GEOID10<-as.factor(censusadjacentyesnodes$src_GEOID10)
censusadjacentyesnodes$nbr_GEOID10<-as.factor(censusadjacentyesnodes$nbr_GEOID10)
...
# save file to disk
save(cayncon,file="cayncon.rdata")
...
monitorct <- cayncon[which(cayncon$src_GEOID10 %in% onlyurbanruca$tract_ID),]
...
aggregatevars<-segindcalcbasis[c("censustract", "DP0080001", "DP0080004", "allbutblack")]
as.numeric(aggregatevars$DP0080001, aggregatevars$DP0080004, aggregatevars$allbutblack)
aggregatedcensus<-aggregate(. ~ censustract, data = aggregatevars, sum)
aggregatedcensus$finalsegregationindex<-aggregatedcensus$DP0080004/aggregatedcensus$DP0080001
aggregatedcensus$GEOID10<-aggregatedcensus$censustract
RI_nodes_urban_tracts<-aggregatedcensus[c("GEOID10","finalsegregationindex")]
RI_nodes_urban_tracts$GEOID10 <- as.factor(RI_nodes_urban_tracts$GEOID10)
colnames(RI_nodes_urban_tracts) <- c("tract_ID","RI_nodes_urban")
View(RI_nodes_urban_tracts)

# save RI for all urban tracts with nodes
save(RI_nodes_urban_tracts, file="RI_nodes_urban_tracts.rdata")
load(file="RI_nodes_urban_tracts.rdata")
...
save(RI_yes_nodes_all_tracts, file="RI_yes_nodes_all_tracts.rdata")
...
save(RI_no_nodes_all_tracts, file="RI_no_nodes_all_tracts.rdata")
...
racialisolationallwithnodesquantile <- within(racialisolationindexallwithnodes, RI_quantile <- as.integer(cut2(isolationindex, quantile(isolationindex, c(0.000000000000000000,0.2,0.4,0.6,0.8,1),na.rm=TRUE)),include.lowest=TRUE))

# check quantiles
table(racialisolationallwithnodesquantile$RI_quantile)
View(racialisolationallwithnodesquantile)




#### split RI into quantiles by region ####

# make new dataframe to edit the full census file
editablecensus<-fullcensus

# pad GEOIDs so 10-digit ones are now 11 digits
editablecensus$GEOID10pad0<-str_pad(editablecensus$GEOID10, 11, pad = "0")

# pull digits for state
editablecensus$state2digits<-substr(editablecensus$GEOID10pad0, start = 1, stop = 2)
...
states_for_isolation_index<-states_and_codes

# make variable name the same
states_for_isolation_index$state2digits<-states_for_isolation_index$State.Code
states_for_isolation_index$State.Code<-NULL
View(states_for_isolation_index)

# add states to editable census
editablecensuswithstates<-merge(editablecensus,states_for_isolation_index,by="state2digits",all.x=TRUE)
View(editablecensuswithstates[,190:198])
View(editablecensuswithstates)

# extract state and GEOID10 from editable census
states_and_GEOIDs<-editablecensuswithstates[,c("state2digits","GEOID10","State.Name","GEOID10pad0")]
View(states_and_GEOIDs)

# read in regions
rawregion<-read.csv(file="uscensusbureauregionsanddivisions.csv")
View(rawregion)
...
states_GEOIDs_and_regions<-merge(states_and_GEOIDs,rawregion,by="State",all.x=TRUE)
View(states_GEOIDs_and_regions)

# make tract_ID for merging
states_GEOIDs_and_regions$tract_ID<-states_GEOIDs_and_regions$GEOID10

# save file before merging with RI so we can merge with all RIs

load(file="racialisolationindexall.rdata")

# merge states, GEOIDs, regions, and RI
racial_iso_region_nodes<-merge(states_GEOIDs_and_regions,racialisolationindexallwithnodes,by="tract_ID",all.x=TRUE)
View(is.na(racial_iso_region_nodes))

# make region into a character
racial_iso_region_nodes$Region<-as.character(racial_iso_region_nodes$Region)

# fix DC to be in the south
racial_iso_region_nodes$Region[racial_iso_region_nodes$State=='District Of Columbia'] <- 'South'

# fix Puerto Rico
racial_iso_region_nodes$Region[racial_iso_region_nodes$State=='Puerto Rico'] <- 'Other'

# make table of isolation NAs before fixing
racial_iso_region_nodes_NAs_for_iso<-racial_iso_region_nodes[is.na(racial_iso_region_nodes$isolationindex),]
save(racial_iso_region_nodes_NAs_for_iso,file="racial_iso_region_nodes_NAs_for_iso.rdata")
View(racial_iso_region_nodes_NAs_for_iso)

# split into quantiles by region
racial_iso_nodes_quantiles_by_region<-racial_iso_region_nodes %>%
  group_by(Region) %>% 
  mutate(RegionQuantile = as.integer(cut2(isolationindex, 
                                 (quantile(isolationindex, probs=0:5/5,na.rm=TRUE)))))

# split into overall quantiles
racial_iso_nodes_quantiles_by_region$OverallQuantile <- as.integer(cut2(racial_iso_nodes_quantiles_by_region$isolationindex, quantile(racial_iso_nodes_quantiles_by_region$isolationindex, probs=0:5/5,na.rm=TRUE)))

View(racial_iso_nodes_quantiles_by_region)

# save and load
save(racial_iso_nodes_quantiles_by_region,file="racial_iso_nodes_quantiles_by_region.rdata")
load(file="racial_iso_nodes_quantiles_by_region.rdata")

#### done to here 2018-04-17 ####
...
monthlyaveragesnodrops3<-aggregate(Arithmetic.Mean ~ month + year + Parameter.Code + tract_ID, atleast3years180obs, mean)

#get long-term averages
averagebycensustractnodrops3<-aggregate(Arithmetic.Mean ~ Parameter.Code + tract_ID, data = monthlyaveragesnodrops3, FUN="mean")

#make it by tract
moltenaverage3<-melt(averagebycensustractnodrops3)
tractaverage3<-dcast(moltenaverage3,tract_ID ~ Parameter.Code)
View(tractaverage3)
...
tractcountbypollutant3<-colSums(!is.na(tractaverage3) | tractaverage3 != "", na.rm = TRUE)
write.csv(tractcountbypollutant3,file="tractcountbypollutant3.csv")
...
tractcountbypollutanturban3<-colSums(!is.na(tractaverageonlyurbanandtotalpop3))
...
threeyearspluswithRInodes<-merge(tractaverageonlyurbanover1003,racialisolationindexallwithnodes,by="tract_ID")
...
save(threeyearspluswithRInodes,file="threeyearspluswithRInodes.rdata")
# load(file="threeyearspluswithRI.rdata")

#### run everything with three years plus with RI ####

# add region and overall quantiles
threeyearsplusregionRInodes<-merge(threeyearspluswithRInodes,racial_iso_nodes_quantiles_by_region,by="tract_ID")
View(threeyearsplusregionRInodes)

table(threeyearsplusregionRInodes$OverallQuantile)
...
threeyearspluswithRInodesquantilesandrace<-merge(threeyearsplusregionRInodes,censusfullracedata,by="tract_ID")
tractcountbypollutant<-colSums(!is.na(threeyearspluswithRInodesquantilesandrace))
...
threeyearspluswithRInodesquantilesandrace$GEOID<-threeyearspluswithRInodesquantilesandrace$tract_ID
...
air_acs_census_seg_nodes_no_drops<-merge(air_acs_seg_nodes_no_drops,agefromcensus,by="GEOID")
air_acs_census_seg_nodes_no_drops<-merge(air_acs_census_seg_nodes_no_drops,HispanicLatinofromCensus,by="GEOID")
air_acs_census_seg_nodes_no_drops<-merge(air_acs_census_seg_nodes_no_drops,raceethnicityfromcensus,by="GEOID")

# note, just replaced old variables starting here
finaldatanodrops<-air_acs_census_seg_nodes_no_drops
...
# % white -- 
finaldatanodrops$percent_white <- finaldatanodrops$DP0080003 / finaldatanodrops$DP0080001
...
# % black
finaldatanodrops$percent_black <- finaldatanodrops$DP0080004 / finaldatanodrops$DP0080001
...
#### PM25 ####
plot(`88101` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88101` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

PM25_88101_levene <- leveneTest(finaldatanodrops$`88101` ~ as.factor(finaldatanodrops$OverallQuantile))
PM25_88101_levene

PM25_88101_anova<- aov(finaldatanodrops$`88101` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(PM25_88101_anova)

PM25_88101_one<-oneway.test(finaldatanodrops$`88101` ~ finaldatanodrops$OverallQuantile)
PM25_88101_one
summary(PM25_88101_one)
TukeyHSD(PM25_88101_anova)
PM25_no_NAs<-finaldatanodrops[c("88101","OverallQuantile")]
PM25_no_NAs<-na.omit(PM25_no_NAs)
posthocTGH(y=PM25_no_NAs$`88101`,x=PM25_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(PM25_no_NAs$OverallQuantile,PM25_no_NAs$`88101`)
...
# simple glm
simpleglmPM25<-glm(finaldatanodrops$`88101`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
PM25summary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
               N    = sum(!is.na(`88101`)),
...
ggplot(PM25summary, aes(x=factor(OverallQuantile), y=mean)) + 
...
plot(`88104` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88104` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

aluminum_88104_levene <- leveneTest(finaldatanodrops$`88104` ~ as.factor(finaldatanodrops$OverallQuantile))
aluminum_88104_levene

aluminum_88104_anova <- aov(finaldatanodrops$`88104` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(aluminum_88104_anova)
aluminum_88104_anova

aluminum_88104_one<-oneway.test(finaldatanodrops$`88104` ~ finaldatanodrops$OverallQuantile, posthoc = 'games-howell')
aluminum_correct_post<-oneway(finaldatanodrops$OverallQuantile, y = finaldatanodrops$`88104`, posthoc = 'games-howell')
...
# glm
simpleglmaluminum_88104<-glm(finaldatanodrops$`88104`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
summary(simpleglmaluminum_88104)

simpleglmaluminumnoregion_88104<-glm(finaldatanodrops$`88104`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$percent_black)
...
# summarize the data
Alsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                     N    = sum(!is.na(`88104`)),
...
ggplot(Alsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88111` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88111` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

calcium_88111_levene <- leveneTest(finaldatanodrops$`88111` ~ as.factor(finaldatanodrops$OverallQuantile))
calcium_88111_levene

calcium_88111_anova <- aov(finaldatanodrops$`88111` ~ as.factor(finaldatanodrops$OverallQuantile)) 
summary(calcium_88111_anova)

calcium_88111_one<-oneway.test(finaldatanodrops$`88111` ~ finaldatanodrops$OverallQuantile)
...
# glm
simpleglmcalcium_88111<-glm(finaldatanodrops$`88111`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region+finaldatanodrops$percent_black)
...
# summarize the data
Casummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88111`)),
...
ggplot(Casummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88115` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88115` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

chlorine_88115_levene <- leveneTest(finaldatanodrops$`88115` ~ as.factor(finaldatanodrops$OverallQuantile))
chlorine_88115_levene

chlorine_88115_anova <- aov(finaldatanodrops$`88115` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(chlorine_88115_anova)
TukeyHSD(chlorine_88115_anova)

chlorine_88115_one<-oneway.test(finaldatanodrops$`88115` ~ finaldatanodrops$OverallQuantile)
TukeyHSD(chlorine_88115_anova)
...
# glm
simpleglmchlorine_88115<-glm(finaldatanodrops$`88115`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Clsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88115`)),
...
ggplot(Clsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88136` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88136` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

nickel_88136_levene <- leveneTest(finaldatanodrops$`88136` ~ as.factor(finaldatanodrops$OverallQuantile))
nickel_88136_levene

nickel_88136_anova <- aov(finaldatanodrops$`88136` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(nickel_88136_anova)

nickel_88136_one<-oneway.test(finaldatanodrops$`88136` ~ finaldatanodrops$OverallQuantile)
nickel_88136_one
TukeyHSD(nickel_88136_anova)

# correct post-hoc
nickel_88136_no_NAs<-finaldatanodrops[c("88136","OverallQuantile")]
nickel_88136_no_NAs<-na.omit(nickel_88136_no_NAs)
posthocTGH(y=nickel_88136_no_NAs$`88136`,x=nickel_88136_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(nickel_88136_no_NAs$OverallQuantile,nickel_88136_no_NAs$`88136`)
...
# glm
simpleglmnickel_88136<-glm(finaldatanodrops$`88136`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Nisummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88136`)),
...
ggplot(Nisummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88161` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88161` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

titanium_88161_levene <- leveneTest(finaldatanodrops$`88161` ~ as.factor(finaldatanodrops$OverallQuantile))
titanium_88161_levene

titanium_88161_anova <- aov(finaldatanodrops$`88161` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(titanium_88161_anova)

titanium_88161_one<-oneway.test(finaldatanodrops$`88161` ~ finaldatanodrops$OverallQuantile)
titanium_88161_one
...
# glm
simpleglmtitanium_88161<-glm(finaldatanodrops$`88161`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Tisummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88161`)),
...
ggplot(Tisummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88164` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88164` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

vanadium_88164_levene <- leveneTest(finaldatanodrops$`88161` ~ as.factor(finaldatanodrops$OverallQuantile))
vanadium_88164_levene

vanadium_88164_anova <- aov(finaldatanodrops$`88164` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(vanadium_88164_anova)

vanadium_88164_one<-oneway.test(finaldatanodrops$`88164` ~ finaldatanodrops$OverallQuantile)
vanadium_88164_one
TukeyHSD(vanadium_88164_anova)

# correct post-hoc
vanadium_88164_no_NAs<-finaldatanodrops[c("88164","OverallQuantile")]
vanadium_88164_no_NAs<-na.omit(vanadium_88164_no_NAs)
posthocTGH(y=vanadium_88164_no_NAs$`88164`,x=vanadium_88164_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(vanadium_88164_no_NAs$OverallQuantile,vanadium_88164_no_NAs$`88164`)
...
# glm
simpleglmvanadium_88164<-glm(finaldatanodrops$`88164`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Vasummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88164`)),
...
ggplot(Vasummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88165` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88165` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

silicon_88165_levene <- leveneTest(finaldatanodrops$`88165` ~ as.factor(finaldatanodrops$OverallQuantile))
silicon_88165_levene

silicon_88165_anova <- aov(finaldatanodrops$`88165` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(silicon_88165_anova)

silicon_88165_one<-oneway.test(finaldatanodrops$`88165` ~ finaldatanodrops$OverallQuantile)
silicon_88165_one
...
# glm
simpleglmsilicon_88165<-glm(finaldatanodrops$`88165`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Sisummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88165`)),
...
ggplot(Sisummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88167` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88167` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

zinc_88167_levene <- leveneTest(finaldatanodrops$`88167` ~ as.factor(finaldatanodrops$OverallQuantile))
zinc_88167_levene

zinc_88167_anova <- aov(finaldatanodrops$`88167` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(zinc_88167_anova)

zinc_88167_one<-oneway.test(finaldatanodrops$`88167` ~ finaldatanodrops$OverallQuantile)
zinc_88167_one
...
# glm
simpleglmzinc_88167<-glm(finaldatanodrops$`88167`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Znsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88167`)),
...
ggplot(Znsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88301` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88301` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

ammonium_88301_levene <- leveneTest(finaldatanodrops$`88301` ~ as.factor(finaldatanodrops$OverallQuantile))
ammonium_88301_levene

ammonium_88301_anova <- aov(finaldatanodrops$`88301` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(ammonium_88301_anova)

ammonium_88301_one<-oneway.test(finaldatanodrops$`88301` ~ finaldatanodrops$OverallQuantile)
ammonium_88301_one
TukeyHSD(ammonium_88301_anova)

# ammonium correct post-hoc
ammonium_88301_no_NAs<-finaldatanodrops[c("88301","OverallQuantile")]
ammonium_88301_no_NAs<-na.omit(ammonium_88301_no_NAs)
posthocTGH(y=ammonium_88301_no_NAs$`88301`,x=ammonium_88301_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(ammonium_88301_no_NAs$OverallQuantile,ammonium_88301_no_NAs$`88301`)
...
# glm
simpleglmammonium_88301<-glm(finaldatanodrops$`88301`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Amsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88301`)),
...
ggplot(Amsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88302` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88302` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

sodium_ion_88302_levene <- leveneTest(finaldatanodrops$`88302` ~ as.factor(finaldatanodrops$OverallQuantile))
sodium_ion_88302_levene

sodium_ion_88302_anova <- aov(finaldatanodrops$`88302` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(sodium_ion_88302_anova)

sodium_ion_88302_one<-oneway.test(finaldatanodrops$`88302` ~ finaldatanodrops$OverallQuantile)
...
# glm
simpleglmsodium_ion_88302<-glm(finaldatanodrops$`88302`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
NaIsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88302`)),
...
ggplot(NaIsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88306` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88306` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

nitrate_88306_levene <- leveneTest(finaldatanodrops$`88306` ~ as.factor(finaldatanodrops$OverallQuantile))
nitrate_88306_levene

nitrate_88306_anova <- aov(finaldatanodrops$`88306` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(nitrate_88306_anova)

nitrate_88306_one<-oneway.test(finaldatanodrops$`88306` ~ finaldatanodrops$OverallQuantile)
nitrate_88306_one
...
# glm
simpleglmnitrate_88306<-glm(finaldatanodrops$`88306`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
NO3summary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                    N    = sum(!is.na(`88306`)),
...
ggplot(NO3summary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88403` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88403` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

sulfate_88403_levene <- leveneTest(finaldatanodrops$`88403` ~ as.factor(finaldatanodrops$OverallQuantile))
sulfate_88403_levene

sulfate_88403_anova <- aov(finaldatanodrops$`88403` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(sulfate_88403_anova)

sulfate_88403_one<-oneway.test(finaldatanodrops$`88403` ~ finaldatanodrops$OverallQuantile)
sulfate_88403_one
TukeyHSD(sulfate_88403_anova)

# sulfate correct post-hoc
sulfate_88403_no_NAs<-finaldatanodrops[c("88403","OverallQuantile")]
sulfate_88403_no_NAs<-na.omit(sulfate_88403_no_NAs)
posthocTGH(y=sulfate_88403_no_NAs$`88403`,x=sulfate_88403_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(sulfate_88403_no_NAs$OverallQuantile,sulfate_88403_no_NAs$`88403`)
...
# glm
simpleglmsulfate_88403<-glm(finaldatanodrops$`88403`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
SO4summary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                    N    = sum(!is.na(`88403`)),
...
ggplot(SO4summary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`99321` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`99321` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

EC_levene <- leveneTest(finaldatanodrops$`99321` ~ as.factor(finaldatanodrops$OverallQuantile))
EC_levene

EC_anova <- aov(finaldatanodrops$`99321` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(EC_anova)

EC_one<-oneway.test(finaldatanodrops$`99321` ~ finaldatanodrops$OverallQuantile)
EC_one
TukeyHSD(EC_anova)
...
# glm
simpleglmEC<-glm(finaldatanodrops$`99321`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
summary(simpleglmEC)
...
# summarize the data
ECsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                    N    = sum(!is.na(`99321`)),
...
ggplot(ECsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88103` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88103` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

arsenic_levene <- leveneTest(finaldatanodrops$`88103` ~ as.factor(finaldatanodrops$OverallQuantile))
arsenic_levene

arsenic_anova <- aov(finaldatanodrops$`88103` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(arsenic_anova)

arsenic_one<-oneway.test(finaldatanodrops$`88103` ~ finaldatanodrops$OverallQuantile)
arsenic_one
TukeyHSD(arsenic_anova)

# arsenic correct post-hoc
arsenic_88103_no_NAs<-finaldatanodrops[c("88103","OverallQuantile")]
arsenic_88103_no_NAs<-na.omit(arsenic_88103_no_NAs)
posthocTGH(y=arsenic_88103_no_NAs$`88103`,x=arsenic_88103_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(arsenic_88103_no_NAs$OverallQuantile,arsenic_88103_no_NAs$`88103`)
...
# glm
simpleglmarsenic<-glm(finaldatanodrops$`88103`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Assummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88103`)),
...
as_plot<-ggplot(Assummary, aes(x=factor(OverallQuantile), y=mean,color=OverallQuantile)) +
...
plot(`88109` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88109` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

bromine_88109_levene <- leveneTest(finaldatanodrops$`88109` ~ as.factor(finaldatanodrops$OverallQuantile))
bromine_88109_levene

bromine_88109_anova <- aov(finaldatanodrops$`88109` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(bromine_88109_anova)

bromine_88109_one<-oneway.test(finaldatanodrops$`88109` ~ finaldatanodrops$OverallQuantile)
bromine_88109_one
TukeyHSD(bromine_88109_anova)

# bromine correct post-hoc
bromine_88109_no_NAs<-finaldatanodrops[c("88109","OverallQuantile")]
bromine_88109_no_NAs<-na.omit(bromine_88109_no_NAs)
posthocTGH(y=bromine_88109_no_NAs$`88109`,x=bromine_88109_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(bromine_88109_no_NAs$OverallQuantile,bromine_88109_no_NAs$`88109`)
...
# glm
simpleglmbromine_88109<-glm(finaldatanodrops$`88109`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Brsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88109`)),
...
ggplot(Brsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88114` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88114` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

copper_88114_levene <- leveneTest(finaldatanodrops$`88114` ~ as.factor(finaldatanodrops$OverallQuantile))
copper_88114_levene

copper_88114_anova <- aov(finaldatanodrops$`88114` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(copper_88114_anova)

copper_88114_one<-oneway.test(finaldatanodrops$`88114` ~ finaldatanodrops$OverallQuantile)
copper_88114_one
TukeyHSD(copper_88114_anova)

# copper correct post-hoc
copper_88114_no_NAs<-finaldatanodrops[c("88114","OverallQuantile")]
copper_88114_no_NAs<-na.omit(copper_88114_no_NAs)
posthocTGH(y=copper_88114_no_NAs$`88114`,x=copper_88114_no_NAs$OverallQuantile,method = 'games-howell',formatPvalue = FALSE)
games.howell(copper_88114_no_NAs$OverallQuantile,copper_88114_no_NAs$`88114`)
...
# glm
simpleglmcopper_88114<-glm(finaldatanodrops$`88114`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Cusummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88114`)),
...
ggplot(Cusummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88126` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88126` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

iron_88126_levene <- leveneTest(finaldatanodrops$`88126` ~ as.factor(finaldatanodrops$OverallQuantile))
iron_88126_levene

iron_88126_anova <- aov(finaldatanodrops$`88126` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(iron_88126_anova)

iron_88126_one<-oneway.test(finaldatanodrops$`88126` ~ finaldatanodrops$OverallQuantile)
iron_88126_one
...
# glm
simpleglmiron_88126<-glm(finaldatanodrops$`88126`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Fesummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88126`)),
...
ggplot(Fesummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88128` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88128` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

lead_88128_levene <- leveneTest(finaldatanodrops$`88128` ~ as.factor(finaldatanodrops$OverallQuantile))
lead_88128_levene

lead_88128_anova <- aov(finaldatanodrops$`88128` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(lead_88128_anova)

lead_88128_one<-oneway.test(finaldatanodrops$`88128` ~ finaldatanodrops$OverallQuantile)
lead_88128_one
...
# glm
simpleglmlead_88128<-glm(finaldatanodrops$`88128`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Pbsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88128`)),
...
ggplot(Pbsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88142` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88142` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

mercury_88142_levene <- leveneTest(finaldatanodrops$`88142` ~ as.factor(finaldatanodrops$OverallQuantile))
mercury_88142_levene

mercury_88142_anova <- aov(finaldatanodrops$`88142` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(mercury_88142_anova)

mercury_88142_one<-oneway.test(finaldatanodrops$`88142` ~ finaldatanodrops$OverallQuantile)
mercury_88142_one
...
# glm
simpleglmmercury_88142<-glm(finaldatanodrops$`88142`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Hgsummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88142`)),
...
ggplot(Hgsummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
plot(`88184` ~ OverallQuantile, data=finaldatanodrops)

boxplot(`88184` ~ OverallQuantile, data=finaldatanodrops,
        xlab = "Treatment", ylab = "Weight",
        frame = FALSE, col = c("#00AFBB", "#E7B800", "#FC4E07"))

sodium_88184_levene <- leveneTest(finaldatanodrops$`88184` ~ as.factor(finaldatanodrops$OverallQuantile))
sodium_88184_levene

sodium_88184_anova <- aov(finaldatanodrops$`88184` ~ as.factor(finaldatanodrops$OverallQuantile))
summary(sodium_88184_anova)

sodium_88184_one<-oneway.test(finaldatanodrops$`88184` ~ finaldatanodrops$OverallQuantile)
sodium_88184_one
...
# glm
simpleglmsodium_88184<-glm(finaldatanodrops$`88184`~finaldatanodrops$OverallQuantile+finaldatanodrops$percent_less_than_high_school+finaldatanodrops$percent_poverty+finaldatanodrops$percent_Hispanic+finaldatanodrops$Region)
...
# summarize the data
Nasummary <- ddply(finaldatanodrops, c("OverallQuantile"), summarise,
                   N    = sum(!is.na(`88184`)),
...
ggplot(Nasummary, aes(x=factor(OverallQuantile), y=mean)) + 
  geom_line(aes(group=1)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) +
  geom_point() +
  xlab("Racial Isolation Quintile") +
...
# monthlyaveragesnodrops<-aggregate(Arithmetic.Mean ~ month + year + Parameter.Code + tract_ID, allairdataandruca, mean)
# 
# 
# #get long-term averages
# averagebycensustractnodrops<-aggregate(Arithmetic.Mean ~ Parameter.Code + tract_ID, data = monthlyaveragesnodrops, FUN="mean")
# 
# #make it by tract
# moltenaverage<-melt(averagebycensustractnodrops)
# 
# tractaverage<-dcast(moltenaverage,tract_ID ~ Parameter.Code)
# 
# #remove OC
...
# #####THREE-YEAR DATA RUN
# #aggregate mean on months and years for three-year data
# monthlyaveragesnodrops3<-aggregate(Arithmetic.Mean ~ month + year + Parameter.Code + tract_ID, lessthan3yearsexcluded, mean)
# 
# #get long-term averages
# averagebycensustractnodrops3<-aggregate(Arithmetic.Mean ~ Parameter.Code + tract_ID, data = monthlyaveragesnodrops3, FUN="mean")
# 
...
# monthlyaveragesnodrops5<-aggregate(Arithmetic.Mean ~ month + year + Parameter.Code + tract_ID, lessthan5yearsexcluded, mean)
# 
# #get long-term averages
# averagebycensustractnodrops5<-aggregate(Arithmetic.Mean ~ Parameter.Code + tract_ID, data = monthlyaveragesnodrops5, FUN="mean")
# 
# #make it by tract
# moltenaverage5<-melt(averagebycensustractnodrops5)
# 
# 
# tractaverage5<-dcast(moltenaverage5,tract_ID ~ Parameter.Code)
# 
# 
# #remove OC
...
#% white -- 
finaldatathree_drop$percent_white <- finaldatathree_drop$DP0080003 / finaldatathree_drop$DP0080001
...
#% black
finaldatathree_drop$percent_black <- finaldatathree_drop$DP0080004 / finaldatathree_drop$DP0080001
...
#make composite variables
#% white -- 
finaldatafive_drop$percent_white <- finaldatafive_drop$DP0080003 / finaldatafive_drop$DP0080001
...
#% black
finaldatafive_drop$percent_black <- finaldatafive_drop$DP0080004 / finaldatafive_drop$DP0080001
...
# 
# 
# aluminum_observations<-subset(compact_air_and_tracts, Parameter.Code == "88104")
...
# #varnames_PM25_all
# #PM25_monitor_preliminary_data<-PM25all[,c("State.Code","County.Code","Site.Num","Latitude","Longitude","Datum","Local.Site.Name","Address","State.Name","County.Name","City.Name","CBSA.Name","ID","Parameter.Code")]
...
# #names(PM_25_monitors)[names(PM_25_monitors) == 'Parameter.Code'] <- 'Monitor.Type'
# 
