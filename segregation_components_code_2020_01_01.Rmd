...
intersected_valid_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = valid_self_intersected_tracts)
...
save(valid_self_intersected_tracts, 
     file = file.path("intermediate_data", 
                      "valid_self_intersected_tracts.rdata"))
...
# safely intersect all buffer sizes for all erroring tracts
zero_buffered_self_intersected_tracts <-
  map(.f = safe_buffer_intersector_nation,
...
intersected_zero_buffered_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = zero_buffered_self_intersected_tracts)
...
save(erroring_tracts_zero_buffered, 
     file = file.path("intermediate_data", "erroring_tracts_zero_buffered.rdata")) 
save(zero_buffered_self_intersected_tracts, 
     file = file.path("intermediate_data", 
                      "zero_buffered_self_intersected_tracts.rdata"))
...
save(precise_intersected_tracts_with_metadata, 
     file = file.path("intermediate_data", "precise_intersected_tracts_with_metadata.rdata"))
...
# run all tracts!
snapped_grid_intersected_tracts <- map(.f = snap_to_grid_minimizer_nation, 
                                       .x = erroring_tracts)
...
# load files
intermediate_object_loader("precise_intersected_tracts_with_metadata", 
                           "snapped_self_intersected_tracts",
                           "snapped_grid_intersected_tracts")

# format precise intersected tracts for easy error finding
tract_intersect_selector <- 
  function(tract_list){
    map(.x = tract_list, 
...
precise_intersected_tracts <- 
  ...
  map(.f = tract_intersect_selector)

rm(precise_intersected_tracts_with_metadata)
...
success_detector_for_tract_list <- 
  function(intersect_output_tract_list){
    successes <- 
      intersect_output_tract_list[is.null(intersect_output_tract_list$error)]
...
# pull tracts from self-intersections


# tracts still erroring after precision and snapping to self at meter tolerance


# pull rest from those snapped to the grid
