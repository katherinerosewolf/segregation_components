...
intermediate_object_saver <- 
  function(...) {
    objects_themselves <- tibble::lst(...)
    object_names <- names(objects_themselves)
    saver <- 
      function(object_itself, object_name){
        save(list = object_name,
             file = file.path("intermediate_data",
                              paste0(object_name,
                                     ".rdata")))
      }
    map2(.x = objects_themselves, 
         .y = object_names,
         .f = saver)
  }
...
intermediate_object_saver(monitors_sf)
...
buffer_tract_intersections_multipolygons <- 
  future_map(.f = function(x, y) st_intersects(y, x), 
             .x = buffers, 
...
buffer_colander <- 
  function(buffer_sf_for_colander,
    ...
    list_by_tract_of_filtered_buffers <- 
      future_map(.f = buffer_filter_function,
                 .x = buffer_sgbp_guide, 
