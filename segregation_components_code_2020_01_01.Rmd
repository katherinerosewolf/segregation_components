...
```{r systematically fixing precision}
...
# make function that generates results and errors after intersecting buffers
safe_intersect <- safely(buffer_intersector_tract)

test_tract <- erroring_tracts$buffer_10000_m$`2906`
  ...
  function(tract_of_buffers_sf){
    ...
    while (error_existence & more_vector_left) {
      ...
      precise_tract <- st_set_precision(tract_of_buffers_sf,
                                        precision = precision_vector[indexer])
      intersect_precise <- safe_intersect(precise_tract)
      error_existence <- !is.null(intersect_precise$error)
      more_vector_left <- (indexer < length(precision_vector))
    }
    
    if(more_vector_left == FALSE & error_existence == TRUE){
      
      return("precision didn't fix it")
      
    } else {
      
      list_to_return <- tibble::lst(precision_vector[indexer],
                                    precise_tract, 
                                    intersect_precise)
...
precision_maximizer_tract_test <- precision_maximizer_tract(test_tract)
...
# run all tracts!
maximally_precise_tracts <- map(.f = precision_maximizer_nation,
                                .x = erroring_tracts)
...
```{r write precision function}

precise_intersector <- 
  function(precision_integer, erroring_tract_list){
    ...
    precise_function <- 
      function(tracts_sf, desired_precision) {
        map(.x = tracts_sf,
            .f = function(x) st_set_precision(x, precision = precision_integer))
      }
    
    
    precise_erroring_tracts <- 
      map(.x = erroring_tract_list, 
          .f = precise_function, 
          desired_precision = precision_integer)
    
    
    # safely intersect all buffer sizes for all erroring tracts
    safely_intersected_tracts <- 
      map(.f = safe_buffer_intersector_nation,
          .x = precise_erroring_tracts,
          buffer_intersector_little_function = buffer_intersector_tract)
    
    
    # isolate tracts still erroring
    remaining_errors <- 
      map(.f = function(x) Filter(Negate(is.null), x$error),
          .x = safely_intersected_tracts)
    
    
    # count errors
    remaining_error_counts <- 
      map(.x = remaining_errors,
          .f = length)

    
    # buffer indices of tracts still erroring
    error_indices <- 
      map(.f = names,
          .x = remaining_errors)
    
    
    results_list <- lst(precise_erroring_tracts, 
                        safely_intersected_tracts, 
                        remaining_errors, 
                        remaining_error_counts, 
                        error_indices)
...
save(precise_intersector, file = file.path("functions", 
                                           "precise_intersector.rdata"))

```



```{r snap tracts to grid}
  ...
  function(tract_of_buffers_sf){
    ...
    while (error_existence) {
      ...
      snapped_tract <- st_snap_to_grid(tract_of_buffers_sf,
                                       size = cell_size)
      intersect_snap <- safe_intersect(snapped_tract)
...
minimizer_test <- snap_to_grid_minimizer_tract(test_tract)
...
# run all tracts!
minimally_snapped_tracts <- map(.f = snap_to_grid_minimizer_nation, 
                                .x = erroring_tracts)
...
intermediate_object_saver("minimally_snapped_tracts")

```



```{r run precise function for desired precisions}

# load functions to save and load intermediate files
load(file = file.path("functions", "intermediate_object_loader.rdata"))
load(file = file.path("functions", "intermediate_object_saver.rdata"))

# load function to make intersections
load(file = file.path("functions", "precise_intersector.rdata"))

# load files
intermediate_object_loader("erroring_tracts")

# make vector of precision options
precision_vector <- c(1000, 100, 10, 1, 0.1, 0.01, 0.001)

# run all the precisions!
precise_results <- 
  map(.f = precise_intersector, 
      .x = precision_vector, 
      erroring_tract_list = erroring_tracts)

# name results by precision value
names(precise_results) <- as.character(precision_vector)

intermediate_object_saver("precise_results")
...
intermediate_object_saver("erroring_tracts_snapped", 
                          "safely_intersected_snapped_erroring_tracts",
                          "snapped_error_counts", 
                          "snapped_error_indices")
...
# precise_results[[1]]$error_indices
...
```{r find remaining errors after each set combined, eval=FALSE, include=FALSE}
