...
# buffclipper_check_intersections <- 
#   function(tract_geoid_as_character, 
#            buffer_sf, 
#            tract_sf) {
#     
...
```{r clipped 2000-meter buffers for all tracts}
...
```{r clipped 2500-meter buffers for all tracts}
...
```{r clipped 3000-meter buffers for all tracts}
...
```{r clipped 4000-meter buffers for all tracts}
...
```{r clipped 5000-meter buffers for all tracts}
...
```{r clipped 7500-meter buffers for all tracts}
...
```{r clipped 10000-meter buffers for all tracts}
...
```{r apply buffintersector to 1000-meter buffers using safely}
...
```{r apply buffintersector to 1000-meter buffers using possibly}
...
```{r isolate the erroring tracts buffintersector 1000-meter}
...
```{r isolate error cases buffintersector 1000-meter}
...
```{r apply buffintersector to 2000-meter buffers using safely}
...
```{r apply buffintersector to 2000-meter buffers using possibly}
...
```{r isolate the erroring tracts buffintersector 2000-meter}
...
```{r isolate error cases buffintersector 2000-meter}
...
```{r apply buffintersector to 2500-meter buffers using safely}
...
```{r apply buffintersector to 2500-meter buffers using possibly}
...
```{r isolate the erroring tracts buffintersector 2500-meter}
...
```{r isolate error cases buffintersector 2500-meter}
...
```{r load remaining errors no change 5}
...
```{r clipped 3000-meter buffers for erroring tracts using buffclipper_precvalbuffonly}
...
```{r clipped 4000-meter buffers for erroring tracts using buffclipper_precvalbuffonly}
...
```{r clipped 5000-meter buffers for erroring tracts using buffclipper_precvalbuffonly}
...
```{r clipped 7500-meter buffers for erroring tracts using buffclipper_precvalbuffonly}
...
```{r clipped 10000-meter buffers for erroring tracts using buffclipper_precvalbuffonly}
...
```{r clipped 3000-meter buffers for erroring tracts using buffclipper_precvaltractbuff}
...
```{r clipped 4000-meter buffers for erroring tracts using buffclipper_precvaltractbuff}
...
```{r clipped 5000-meter buffers for erroring tracts using buffclipper_precvaltractbuff}
...
```{r clipped 7500-meter buffers for erroring tracts using buffclipper_precvaltractbuff}
...
```{r clipped 10000-meter buffers for erroring tracts using buffclipper_precvaltractbuff}
...
```{r apply buffintersector to 3000-meter buffers using safely 2}
...
```{r apply buffintersector to 3000-meter buffers using possibly 2}
...
```{r isolate the erroring tracts buffintersector 3000-meter 2}
...
```{r apply buffintersector to 4000-meter buffers using safely 2}
...
```{r apply buffintersector to 4000-meter buffers using possibly 2}
...
```{r isolate the erroring tracts buffintersector 4000-meter 2}
...
```{r apply buffintersector to 5000-meter buffers using safely 2}
...
```{r apply buffintersector to 5000-meter buffers using possibly 2}
...
```{r isolate the erroring tracts buffintersector 5000-meter 2}
...
```{r apply buffintersector to 7500-meter buffers using safely 2}
...
```{r apply buffintersector to 7500-meter buffers using possibly 2}
...
```{r isolate the erroring tracts buffintersector 7500-meter 2}
...
```{r apply buffintersector to 10000-meter buffers using safely 2}
...
```{r apply buffintersector to 10000-meter buffers using possibly 2}
...
```{r isolate the erroring tracts buffintersector 10000-meter 2}
...
```{r load remaining errors no change 6}
...
```{r apply buffintersector to 3000-meter buffers using safely 3}
...
```{r apply buffintersector to 3000-meter buffers using possibly 3}
...
```{r isolate the erroring tracts buffintersector 3000-meter 3}
...
```{r apply buffintersector to 5000-meter buffers using safely 3}
...
```{r apply buffintersector to 5000-meter buffers using possibly 3}
...
```{r isolate the erroring tracts buffintersector 5000-meter 3}
...
```{r apply buffintersector to 7500-meter buffers using safely 3}
...
```{r apply buffintersector to 7500-meter buffers using possibly 3}
...
```{r isolate the erroring tracts buffintersector 7500-meter 3}
...
```{r apply buffintersector to 10000-meter buffers using safely 3}
...
```{r apply buffintersector to 10000-meter buffers using possibly 3}
...
```{r isolate the erroring tracts buffintersector 10000-meter 3}
