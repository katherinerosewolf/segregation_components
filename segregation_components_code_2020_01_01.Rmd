...
not_null_buffer_intersection_errors <- 
  map(
...
error_counts_by_buffer_size <- 
  map(
    .x = not_null_buffer_intersection_errors,
    .f = length
  )
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "not_null_buffer_intersection_errors.rdata"
    )
)
...
```{r snap erroring tracts to themselves}
...
# get buffer indices of erroring tracts
error_tract_indices <-
  map(
    .f = names,
    .x = not_null_buffer_intersection_errors
...
# snap them to themselves
snap_tracts_one_buffer_size <- 
  function(tracts_in_one_buffer_size) {
    map(
      .x = tracts_in_one_buffer_size,
      .f = function(x) st_snap(x, x, tolerance = 10)
    )
  }

# snap the tracts and try again
snapped_erroring_tracts <- 
  map(
    .x = erroring_tracts, 
    .f = snap_tracts_one_buffer_size
  )

# try again

# safely intersect all buffer sizes for all erroring tracts
safely_intersected_buffers_snapped_erroring_tracts <- 
  future_map(
    .f = safe_buffer_intersector_nation, 
...
# save the snapped erroring tracts
save(
  snapped_erroring_tracts,
  file =
    file.path(
      "intermediate_data",
      "snapped_erroring_tracts.rdata"
    )
)
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "safely_intersected_buffers_snapped_erroring_tracts.rdata"
      )
  )

# investigate errors
snapped_not_null_buffer_intersection_errors <- 
  map(
    .f = function(x) Filter(Negate(is.null), 
                            x$error),
    .x = safely_intersected_buffers_snapped_erroring_tracts
  )

snapped_error_counts_by_buffer_size <- 
  map(
    .x = snapped_not_null_buffer_intersection_errors,
    .f = length
  )
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "snapped_not_null_buffer_intersection_errors.rdata"
    )
)
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "snapped_error_counts_by_buffer_size.rdata"
    )
)
