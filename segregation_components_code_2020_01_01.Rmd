...
```{r buffclipper after intersections testing in al, eval=FALSE, include=FALSE}
...
tracts_clipped_buffers_1000_m <- 
  future_map2(
    .f = buffer_clipper_single_tract, 
    .x = al_tracts_simple_list, 
    .y = buffers_by_tract_1000_m_sf_list
  )
...
```{r buffclipper after intersections for realsies}
...
# make tract sf into a list of single-feature sfs
tracts_simple_list <- 
  split(projected_tracts_simple, 
        seq(nrow(projected_tracts_simple)))
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "tracts_clipped_buffers_1000_m.rdata"
    )
)
