...
projected_tracts <- 
  ...
  rowid_to_column("multipolygon_tract_id")
...
projected_tracts_simple <- 
  projected_tracts %>% 
  st_cast("POLYGON") %>% 
  rowid_to_column("polygon_tract_id")
...
buffers <-
  map(
    .f = function(x) {
      ...
      rowid_to_column(x, "buffer_index")
      return(x)
      },
    .x = buffers
  )
...
al_tracts <- 
  ...
  rowid_to_column("al_polygon_tract_id")

# isolate simple polygon alabama tracts
al_tracts_simple <- 
  projected_tracts_simple %>% 
  filter(str_detect(geoid10, "^01.*")) %>% 
  rowid_to_column("al_multipolygon_tract_id")

# isolate alabama buffers
al_buffers <-
  map(
    .f = function(x) filter(x, str_detect(x$monitor_id, "^01.*")), 
    .x = buffers
  )
...
```{r calculate buffer intersections with tracts}
...
```{r buffclipper after intersections, eval=FALSE, include=FALSE}
...
# # selects the first tract, gives indices of the buffers that intersect it
# al_intersection_one_tract_its_buffers <- 
  


# buffers_1000_m_in_single_tract <- 
#   # clip buffers to only the one tract
#       buffers_in_tract <-
#         one_buffer_size %>%
#         st_intersection(one_tract, .) %>%  # this clips the buffers (buffer sf is .)
