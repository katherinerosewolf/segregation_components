...
NOTES FOR KATIE: FIGURE OUT IF CONVERSION TO SINGLE POLYGONS IS CAUSING ERRORS
...
intermediate_object_saver("tracts_polygon")
rm(tracts_polygon)
gc()
 
intermediate_object_saver("tracts_polygon_list")
...
# save files
intermediate_object_saver("tracts_multipolygon",
                          "tracts_multipolygon_list")
...
# load buffers
load(file = file.path("intermediate_data", "buffers.rdata"))
...
# save result
save(unionized_buffers, file = file.path("intermediate_data", 
...
# save result
intermediate_object_saver("buffer_tract_intersections_polygons")
...
```{r calculate buffer intersections with multipolygon tracts}
...
# load files
intermediate_object_loader("buffers", 
                           "tracts_multipolygon")  # multipolygon tracts

# calculate for all buffer sizes for multipolygons
buffer_tract_intersections_multipolygons <- 
  map(.f = function(x, y) st_intersects(y, x), 
             .x = buffers, 
             y = tracts_multipolygon)

# save result
intermediate_object_saver("buffer_tract_intersections_multipolygons")
...
# save the above
intermediate_object_saver("buffers_by_tract_all_sizes")
...
intermediate_object_loader("buffer_tract_intersections_polygons", 
                           "buffers_by_tract_all_sizes", 
                           "tracts_polygon_list")

# make buffer clipper function
buffer_clipper_tract <- 
  function(tract_sf, 
           buffers_by_tract_sf){
...
buffer_clipper_nation <- 
  function(buffers_by_tract_list,
           tract_list,
           buffer_clipper_little_function){
    clipped_buffers <- 
      map2(
        .f = buffer_clipper_little_function, 
        .x = tract_list, 
...
clipped_buffers <- 
  map(
    .x = buffers_by_tract_all_sizes, 
    .f = buffer_clipper_nation, 
    tract_list = tracts_polygon_list, 
    buffer_clipper_little_function = buffer_clipper_tract #, 
    # .progress = TRUE
  )
...
# save objects
save(buffer_clipper_tract, file = file.path("functions", 
                                            "buffer_clipper_tract.rdata"))

save(buffer_clipper_nation, file = file.path("functions", 
                                            "buffer_clipper_nation.rdata"))

intermediate_object_saver("clipped_buffers")
...
```{r buffer intersector functions}

# load functions to get the other files
load(file = file.path("functions", "intermediate_object_loader.rdata"))
load(file = file.path("functions", "intermediate_object_saver.rdata"))

# load clipped buffers
...
# buffer intersection function for the nation
buffer_intersector_nation <- 
  function(buffers_clipped_to_tracts,
           buffer_intersector_little_function){
    buffer_intersections_nation <- 
      map(
        .x = buffers_clipped_to_tracts,
        .f = buffer_intersector_tract
      )
    return(buffer_intersections_nation)
  }

# buffer intersection function for the nation that works for all buffers
...
safely_intersected_buffers <- 
  map(
    .f = safe_buffer_intersector_nation, 
    .x = clipped_buffers,
...
save(
  ...
  file = 
    file.path(
      "functions", 
      "buffer_intersector_nation.rdata"
    )
)
...
intermediate_object_saver("safely_intersected_buffers")
...
```{r investigating intersected buffers}
...
intermediate_object_saver("buffer_intersection_errors")
...
# save files
intermediate_object_saver("error_counts_by_buffer_size", 
                          "error_tract_indices", 
                          "erroring_tracts")
...
```{r try setting precision to one meter}
...
intermediate_object_loader("buffer_intersection_errors", 
                           "erroring_tracts")

# make function to set their precision to 1
precisor_one_function <- 
  function(buffers_by_tract) {
    map(.x = buffers_by_tract,
        .f = function(x) st_set_precision(x, precision = 1))
  }

# make tracts precise and try again
erroring_tracts_made_precise_one <- 
  map(.x = erroring_tracts, 
      .f = precisor_one_function)
  
# safely intersect all buffer sizes for all erroring tracts
safely_intersected_precise_one_tracts <- 
  map(.f = safe_buffer_intersector_nation, 
      .x = erroring_tracts_made_precise_one,
      buffer_intersector_little_function = buffer_intersector_tract #, 
    # .progress = TRUE
    )

# select the tracts still erroring
intersected_precise_one_errors <- 
  map(.f = function(x) Filter(Negate(is.null), 
                              x$error),
      .x = safely_intersected_precise_one_tracts)

# count the tracts still erroring
precise_one_error_counts <- 
  map(.x = intersected_precise_one_errors,
      .f = length)

# get buffer indices of tracts still erroring
precise_one_tracts_still_erroring_indices <-
  map(.f = names,
      .x = intersected_precise_one_errors)

# select the tracts still erroring
precise_one_tracts_still_erroring <-
  map2(.x = erroring_tracts, 
      .y = precise_one_tracts_still_erroring_indices,
      .f = function(x, y) x[y])

# save results
intermediate_object_saver("precise_one_error_counts",
                          "precise_one_tracts_still_erroring_indices",
                          "safely_intersected_precise_one_tracts",
                          "precise_one_tracts_still_erroring")
...
end goal: weights from each buffer in each census tract


```{r snap tracts to themselves one-meter tolerance}
...
# do the snapping
snapped_erroring_tracts <-
  map(.x = erroring_tracts,
      .f = tract_snapper)

# safely intersect all buffer sizes for all erroring tracts
safely_intersected_snapped_erroring_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = snapped_erroring_tracts,
      buffer_intersector_little_function = buffer_intersector_tract #,
    # .progress = TRUE
  )
...
intersected_snapped_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = safely_intersected_snapped_erroring_tracts)
...
# get buffer indices of tracts still erroring
snapped_tracts_still_erroring_indices <-
  map(
    .f = names,
    .x = intersected_snapped_errors
  )
...
tracts_still_erroring_after_snapping <-
  map2(
    .x = erroring_tracts, 
    .y = snapped_tracts_still_erroring_indices,
    .f = function(x, y) x[y]
  )

intermediate_object_saver("snapped_erroring_tracts", 
                          "safely_intersected_snapped_erroring_tracts", 
                          "intersected_snapped_errors",
                          "snapped_error_counts", 
                          "snapped_tracts_still_erroring_indices", 
...
# load files
intermediate_object_loader("tracts_still_erroring_after_snapping", 
                           "precise_one_tracts_still_erroring", 
                           "snapped_tracts_still_erroring_indices", 
                           "precise_one_tracts_still_erroring_indices")

# find tracts still erroring in both sets
tracts_erroring_post_snap_precise <- 
  map2(.x = tracts_still_erroring_after_snapping,
       .y = precise_one_tracts_still_erroring, 
       .f = intersect)

# indices of tracts still erroring
indices_tracts_erroring_post_snap_precise <- 
  map2(.x = snapped_tracts_still_erroring_indices,
       .y = precise_one_tracts_still_erroring_indices, 
       .f = intersect)

# counts of tracts still erroring
post_snap_precise_error_counts <- 
  map(.x = indices_tracts_erroring_post_snap_precise, 
      length)

intermediate_object_saver("tracts_erroring_post_snap_precise", 
                          "indices_tracts_erroring_post_snap_precise", 
                          "post_snap_precise_error_counts")
...
# precision 10
precise_point_one_function <- 
  function(buffers_by_tract) {
    map(.x = buffers_by_tract,
        .f = function(x) st_set_precision(x, precision = 0.1))
  }

# make tracts precise and try again
erroring_tracts_precise_point_one <- 
  map(.x = erroring_tracts, 
      .f = precise_point_one_function)

# safely intersect all buffer sizes for all erroring tracts
safely_intersected_precise_point_one_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = erroring_tracts_precise_point_one,
      buffer_intersector_little_function = buffer_intersector_tract #,
    # .progress = TRUE
  )

# investigate errors
precise_point_one_remaining_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = safely_intersected_precise_point_one_tracts)

# count errors
precise_point_one_error_counts <-
  map(.x = precise_point_one_remaining_errors,
      .f = length)

# get buffer indices of tracts still erroring
precise_point_one_still_erroring_indices <-
  map(.f = names,
      .x = precise_point_one_remaining_errors)

# indices of tracts still erroring
indices_tracts_erroring_post_snap_precise_one_point_one <- 
  map2(.x = indices_tracts_erroring_post_snap_precise,
       .y = precise_point_one_still_erroring_indices, 
       .f = intersect)

# counts of tracts still erroring
counts_erroring_post_snap_precise_one_point_one <- 
  map(.x = indices_tracts_erroring_post_snap_precise_one_point_one, 
      length)
...
# precision 100
precise_point_zero_one_function <- 
  function(buffers_by_tract) {
    map(.x = buffers_by_tract,
        .f = function(x) st_set_precision(x, precision = 0.01))
  }

# make tracts precise and try again
erroring_tracts_precise_point_zero_one <- 
  map(.x = erroring_tracts, 
      .f = precise_point_zero_one_function)

# safely intersect all buffer sizes for all erroring tracts
safely_intersected_precise_point_zero_one_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = erroring_tracts_precise_point_zero_one,
      buffer_intersector_little_function = buffer_intersector_tract #,
    # .progress = TRUE
  )

# investigate errors
precise_point_zero_one_remaining_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = safely_intersected_precise_point_zero_one_tracts)

# count errors
precise_point_zero_one_error_counts <-
  map(.x = precise_point_zero_one_remaining_errors,
      .f = length)

# get buffer indices of tracts still erroring
precise_point_zero_one_still_erroring_indices <-
  map(.f = names,
      .x = precise_point_zero_one_remaining_errors)

# indices of tracts still erroring
indices_tracts_erroring_post_snap_precise_one_point_one_zero_one <- 
  map2(.x = indices_tracts_erroring_post_snap_precise_one_point_one,
       .y = precise_point_zero_one_still_erroring_indices, 
       .f = intersect)

# counts of tracts still erroring
counts_erroring_post_snap_precise_one_point_one_zero_one <- 
  map(.x = indices_tracts_erroring_post_snap_precise_one_point_one_zero_one, 
      length)
...
# precision 100
precise_ten_function <- 
  function(buffers_by_tract) {
    map(.x = buffers_by_tract,
        .f = function(x) st_set_precision(x, precision = 10))
  }

# make tracts precise and try again
erroring_tracts_precise_ten <- 
  map(.x = erroring_tracts, 
      .f = precise_ten_function)

# safely intersect all buffer sizes for all erroring tracts
safely_intersected_precise_ten_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = erroring_tracts_precise_ten,
      buffer_intersector_little_function = buffer_intersector_tract #,
    # .progress = TRUE
  )

# investigate errors
precise_ten_remaining_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = safely_intersected_precise_ten_tracts)

# count errors
precise_ten_error_counts <-
  map(.x = precise_ten_remaining_errors,
      .f = length)

# get buffer indices of tracts still erroring
precise_ten_still_erroring_indices <-
  map(.f = names,
      .x = precise_ten_remaining_errors)

# indices of tracts still erroring
indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten <- 
  map2(.x = indices_tracts_erroring_post_snap_precise_one_point_one_zero_one,
       .y = precise_ten_still_erroring_indices, 
       .f = intersect)

# counts of tracts still erroring
counts_erroring_post_snap_precise_one_point_one_zero_one_ten <- 
  map(.x = indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten, 
      length)
...
precise_100_function <- 
  function(buffers_by_tract) {
    map(.x = buffers_by_tract,
        .f = function(x) st_set_precision(x, precision = 100))
  }

# make tracts precise and try again
erroring_tracts_precise_100 <- 
  map(.x = erroring_tracts, 
      .f = precise_100_function)

# safely intersect all buffer sizes for all erroring tracts
safely_intersected_precise_100_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = erroring_tracts_precise_100,
      buffer_intersector_little_function = buffer_intersector_tract #,
    # .progress = TRUE
  )

# investigate errors
precise_100_remaining_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = safely_intersected_precise_100_tracts)

# count errors
precise_100_error_counts <-
  map(.x = precise_100_remaining_errors,
      .f = length)

# get buffer indices of tracts still erroring
precise_100_still_erroring_indices <-
  map(.f = names,
      .x = precise_100_remaining_errors)

# indices of tracts still erroring
indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten_100 <- 
  map2(.x = indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten,
       .y = precise_100_still_erroring_indices, 
       .f = intersect)

# counts of tracts still erroring
counts_erroring_post_snap_precise_one_point_one_zero_one_ten_100 <- 
  map(
    .x = indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten_100, 
    length)
...
# precision one kilometer (jeepers)
precise_point_001_function <- 
  function(buffers_by_tract) {
    map(.x = buffers_by_tract,
        .f = function(x) st_set_precision(x, precision = 0.001))
  }

# make tracts precise and try again
erroring_tracts_precise_point_001 <- 
  map(.x = erroring_tracts, 
      .f = precise_point_001_function)

# safely intersect all buffer sizes for all erroring tracts
safely_intersected_precise_point_001_tracts <-
  map(.f = safe_buffer_intersector_nation,
      .x = erroring_tracts_precise_point_001,
      buffer_intersector_little_function = buffer_intersector_tract #,
    # .progress = TRUE
  )

# investigate errors
precise_point_001_remaining_errors <-
  map(.f = function(x) Filter(Negate(is.null),
                              x$error),
      .x = safely_intersected_precise_point_001_tracts)

# count errors
precise_point_001_error_counts <-
  map(.x = precise_point_001_remaining_errors,
      .f = length)

# get buffer indices of tracts still erroring
precise_point_001_still_erroring_indices <-
  map(.f = names,
      .x = precise_point_001_remaining_errors)

# indices of tracts still erroring
indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten_100_001 <- 
  map2(.x = indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten_100,
       .y = precise_point_001_still_erroring_indices, 
       .f = intersect)

# counts of tracts still erroring
counts_erroring_post_snap_precise_one_point_one_zero_one_ten_100_001 <- 
  map(.x = indices_tracts_erroring_post_snap_precise_one_point_one_zero_one_ten_100_001, 
      length)
