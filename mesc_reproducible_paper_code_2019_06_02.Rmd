...
#### buffer work using full shapefiles
...
```{r clippeed 1000-meter buffers for all tracts}
...
```{r clippeed 2000-meter buffers for all tracts}
...
```{r clippeed 2500-meter buffers for all tracts}
...
```{r clippeed 3000-meter buffers for all tracts}
...
```{r clippeed 4000-meter buffers for all tracts}
...
```{r clippeed 5000-meter buffers for all tracts}
...
```{r clippeed 7500-meter buffers for all tracts}
...
#### make buff tract intersector functions for debugging

```{r make buffer intersection function without fixes from the above single case}

buffintersectintractor_simple <- 
...
rm(buffintersectintractor_simple)
invisible(gc())
...
##### simple bufferintersectintractor

```{r apply buffintersectintractor_simple to 10000-meter buffers using safely}
...
# do 10000-meter file
safely_intersects_simple_10000_m <- 
  map(
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "safely_intersects_simple_10000_m.rdata"
    )
)
...
```{r apply buffintersectintractor_simple to 10000-meter buffers using possibly}
...
# do 10000-m file
possibly_intersects_simple_10000_m <- 
  map(
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "possibly_intersects_simple_10000_m.rdata"
    )
)
...
```{r isolate the erroring tracts buffintersectintractor_simple 10000-meter}
...
errors_simple_10000_m_true_false <- 
  possibly_intersects_simple_10000_m %>% 
  str_detect(pattern = "error")

# make list of just those tracts
errors_simple_10000_m <- 
  possibly_intersects_simple_10000_m %>% 
  subset(errors_simple_10000_m_true_false)
...
```{r isolate error cases buffintersectintractor_simple 10000-meter}
...
buffers_clips_simple_10000_m_throwing_errors <- 
  buffer_clips_10000_m[tracts_simple_10000_m_errors]

# select the error messages
error_messages_simple_10000_m <- 
  safely_intersects_simple_10000_m$error[tracts_simple_10000_m_errors]
...
```{r apply buffintersectintractor using safely}

# load buffintersectintractor function
load(
  file = 
    file.path(
      "functions", 
      "buffintersectintractor.rdata"
      )
  )
...
safely_intersects_1000_m <- 
  map(
  .f = safely(buffintersectintractor), 
...
```{r apply buffintersectintractor using possibly}
...
possibly_intersects_1000_m <- 
  map(
  .f = possibly(buffintersectintractor, 
                otherwise = "error"), 
  .x = buffer_clips_1000_m
)
