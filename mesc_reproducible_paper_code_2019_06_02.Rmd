...
# apply buffer function for all tracts for one buffer size
buffer_intersections_1000_m <- 
  map(
    .f = buffintersector, 
    .x = tract_geoid_characters,
    buffer_sf = buffer_1000_m,
    tract_sf = projected_tracts
  )

# make map_buff_intersect function!
mapbuffintersector <- 
  function(input_buffer_sf) {
    intersections_all_tracts_one_buffer_size <- 
      map(
        .f = buffintersector, 
        .x = tract_geoid_characters,
        buffer_sf = input_buffer_sf,
        tract_sf = projected_tracts
  )
