...
# make simple monitor sf
projected_simple_monitors <- 
  projected_monitors %>% 
  select(
    monitor_id, 
    monitor_type
...
# make simple tract sf
projected_simple_tracts <- 
  projected_tracts %>% 
  select(
    geoid10, 
    aland10, 
...
load(
  file = 
    file.path(
      "intermediate_data", 
      "projected_simple_monitors.rdata"
    )
  )

# make list in which to store the projected monitor sf
list_of_proj_mon_sf <- 
  list(projected_simple_monitors)
...
list_for_projected_tract_sf <- 
  list(projected_simple_tracts)

# remove projected tracts from memory
rm(projected_simple_tracts)
gc()
...
```{r create tract monitor intersections}
...
buffer_tract_intersections$buffer_tract_area_1000_m
