...
```{r census shapefile with dp1 data, results = "hide"}
...
```{r census detailed shapefile without data}
...
```{r census less detailed clipped cartographic boundary file}
...
```{r calculating adjacency purely in R}
...
neighbors <- 
  poly2nb(tigris_shapefiles_no_data, 
          queen = TRUE, 
          row.names = "GEOID10")
...
neighbor_list <- 
  nb2listw(neighbors, zero.policy = TRUE)
```


```{r calculating segregation NOTE: USES ARCGIS, FIX THIS}
...
load(
   file = 
      file.path(
        "intermediate_data",
        "sf1_totalcensus_raw_tibble.rdata"
        )
   )
...
# downloaded 2019 June 18
# last updated per website 2012 September 27
...
if(!
  ...
  state_abbreviation_character_vector <- 
    c("AK", "AL", "AR", "AZ", "CA", "CO", "CT", "DC", "DE", "FL", "GA", "HI", "IA", "ID", "IL", "IN", "KS", "KY", "LA", "MA", "MD", "ME", "MI", "MN", "MO", "MS",  "MT", "NC", "ND", "NE", "NH", "NJ", "NM", "NV", "NY", "OH", "OK", "OR", "PA", "RI", "SC", "SD", "TN", "TX", "UT", "VA", "VT", "WA", "WI", "WV", "WY", "PR")
  ...
  sf1_totalcensus_raw_dataframe <-
    read_decennial(
      year = 2010,
      states = state_abbreviation_character_vector_no_pr,
      table_contents =
        sf1_totalcensus_var_vec,
      summary_level = "tract"
  )
...
# downloaded 2019 June 18
# last update per website 2015 Feburary 27
...
if(!
   file.exists(
     file.path(
       "intermediate_data", 
       "acs_totalcensus_raw_tibble.rdata"
     )
   )
   ) {
  ...
  acs_totalcensus_raw_tibble <- 
    as_tibble(acs_totalcensus_raw_dataframe)
  
  # save raw tibble to disk
  save(
    acs_totalcensus_raw_tibble, 
    file = 
      file.path(
...
```{r speciation download by implementing speciator function en masse}
...
non_us_state_codes <- 
  c(
    # "72",  # PR (now including)
    # "78",  # Virgin Islands (now including)
    "CC",  # Canada
    "80"   # Mexico
    )
...
# remove the filtered air tibble
rm(us_filtered_air_tibble)
gc()
...
# load tibble for monitor counts
load(
  file = 
    file.path(
      "intermediate_data",
...
save(
  ...
  file = 
    file.path(
      "intermediate_data", 
      "monitor_location_data_marked.rdata"
      )
  )
...
# join tracts to monitors (CHECK AGAINST ARCGIS)
monitors_to_tracts <- 
  st_join(
    projected_monitors, 
    projected_tracts
...
# monitor buffer!
buffer_monitors_one_km <- 
  projected_monitors %>% 
  st_buffer(dist = 1000)

# join buffers to tracts
...
### joining the above back to census data

```{r join tracts to census data}
...
nrow(dates_by_monitor_species_only)
table(dates_by_monitor_pm_only$monitor_type)
table(dates_by_monitor_species_only$monitor_type)
...
# BEGIN TRAVIS-FRIENDLY CODE

## ruca

```{r ruca data}

# source https://www.ers.usda.gov/webdocs/DataFiles/53241/ruca2010.xlsx?v=0
# downloaded 2019 June 11
# last updated 12/31/2013 per https://www.ers.usda.gov/data-products/rural-urban-commuting-area-codes.aspx
