...
```{r ruca data, eval=FALSE, include=FALSE}
...
sf1_tibble_with_acs_concordance <- 
  sf1_totalcensus_raw_tibble %>%   # summon the census
...
```{r join tracts to census data, eval=FALSE, include=FALSE}
...
little_monitors_to_tracts <- 
  ...
  select(monitor_id, 
         geoid) %>% 
  mutate(geoid_containing_tract = geoid) %>% 
  select(monitor_id, 
...
```{r process ruca data, eval=FALSE, include=FALSE}
