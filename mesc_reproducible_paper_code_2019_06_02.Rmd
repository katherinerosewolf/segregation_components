...
#     file.path(
#       "intermediate_data", 
#       "buffer_tract_intersection_tibbles.rdata"
#     )
# )
...
```{r figuring out how to get intersections within a tract}
...
one_tract <- 
  projected_tracts %>% 
  filter(geoid10 == "04005002300")

# clip buffers to only the one tract
buffer_intersections_with_the_tract <- 
  one_buffer_size %>% 
  st_intersection(one_tract, .) %>% 
  mutate(polygon_id = row_number())  # add row numbers for id in collowing step
...
within_tract_intersections <- 
  buffer_intersections_with_the_tract %>% 
  st_intersection()

str(buffer_intersections_with_the_tract)

# add areas to them
within_tract_intersection_areas <- 
  within_tract_intersections %>% 
