...
bufferizer(buffer_list = buffers, 
           point_sf = projected_monitors,
           distance = 1000)
...
```{r simple join monitors to tracts, eval=FALSE, include=FALSE}
...
# make empty list for buffers and tracts
buffers_tracts <- 
  list()
...
joiner <- 
  function(buffer_sf,
    ...
    item_name <- 
      str_c(
        deparse(substitute(buffer_sf)), 
        "_buffer_tract", 
        sep = ""
      )
    
    buffer_tract_list[[item_name]] <- 
      st_join(
        buffer_sf,
        tract_sf,
        left = TRUE
...
joiner(
  ...
  buffer_tract_list = buffers_tracts
)
