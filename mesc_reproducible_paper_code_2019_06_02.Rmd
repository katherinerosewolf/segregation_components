...
```{r census cartographic boundary file without data, eval=FALSE, include=FALSE}
...
if(
  ...
  # create geoid10 to match full shapefiles
  tigris_cbfs_no_data <- 
    tigris_cbfs_no_data %>% 
    mutate(
      geoid10 = 
...
if(
  ...
  cbf_tract_order_character <- 
    as.character(1:nrow(tigris_cbfs_no_data))
  ...
  if(!file.exists(file.path("pre_travis_intermediate_data", 
    ...
    cbf_tract_neighbor_nb <-  
      poly2nb(pl = tigris_cbfs_no_data, 
              queen = TRUE)
...
```{r make buffer intersection function from the above single case, eval=FALSE, include=FALSE}
...
```{r apply buffer function to a single case, eval=FALSE, include=FALSE}
...
```{r intra-tract buffer intersections all tracts one distance, eval=FALSE, include=FALSE}
...
```{r cbf intra-tract 1000-meter buffer intersections}
...
```{r cbf intra-tract 2000-meter buffer intersections}
...
```{r cbf intra-tract 2500-meter buffer intersections}
...
```{r cbf intra-tract 3000-meter buffer intersections}
...
```{r cbf intra-tract 4000-meter buffer intersections}
...
```{r cbf intra-tract 5000-meter buffer intersections}
...
```{r cbf intra-tract 7500-meter buffer intersections}
...
```{r cbf intra-tract 10000-meter buffer intersections}
